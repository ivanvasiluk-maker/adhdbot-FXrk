...
def _detailed_analysis_scripts(pattern: str, evidence: List[str], core_hypothesis: str, signals: Optional[Dict[str, Any]] = None, recommended_variant: str = "", skills_focus: Optional[List[str]] = None) -> Dict[str, str]:
    ...
    if signals.get("fear_bad") or signals.get("fear_visible") or pattern in {"perfectionism_visibility_fear", "anxious_fear_error"}:
        ...
        why = "Рабочая гипотеза сейчас: вход ломает страх ошибки или оценки, потому что в данных есть сигнал высокой цены плохого результата."
        unknown = "Неизвестно, что сильнее: страх сделать плохо, перегруз от масштаба задачи или тревога из-за неопределённости."
        ...
        why = "Рабочая гипотеза сейчас: быстрые стимулы становятся способом снять напряжение, потому что внимание уходит туда, где облегчение доступнее задачи."
        unknown = "Неизвестно, телефон здесь главный источник облегчения или просто самый доступный способ уйти от перегруза."
        ...
        why = "Рабочая гипотеза сейчас: вход слишком крупный, потому что задача выглядит не как один физический шаг, а как большой ком."
        unknown = "Неизвестно, что сильнее: масштаб задачи, срочность, усталость или страх выбрать неправильно."
    else:
        start_thought = "«надо начать, но вход выглядит дорогим»"
        avoid = "появляется пауза, подготовка или откладывание"
        why = "Рабочая гипотеза сейчас: вход в задачу слишком дорогой; данных хватает только на осторожную рабочую модель."
        unknown = "Неизвестно, что является главной причиной: страх ошибки, перегруз, тревога, низкий ресурс или быстрые отвлечения."

    skill_name = _skill_display_name(recommended_variant)
    known_lines = [
        "проблема, похоже, возникает не только в самой работе, а в моменте входа",
        "первым тестом нужен маленький шаг вместо требования «соберись»",
    ]
    if signals.get("escapes") or signals.get("attention_escape"):
        known_lines.insert(1, "отвлечение даёт краткое облегчение в текущей рабочей модели")
    if len(facts) < 3:
    ...
    return {
        ...
        "skinny": "Не гадаем. За три вопроса выясним, где именно ломается вход, и выберем шаг.\n\n" + base,
