...
from db import (
    ...
    get_value_proof_metrics,
    startup_schema_check,
    record_behavioral_analytics_event,
    create_situation_snapshot, create_mechanism_hypothesis, create_behavioral_experiment,
...
# The day close is a short behavioural diary card, not a generic "how was
# your day" prompt.  Keep it small enough for an ADHD-facing interface while
# still separating the function that broke, the likely barrier and state.
kb_day_review_function = ReplyKeyboardMarkup(
    keyboard=[
...
kb_day_review_barrier = ReplyKeyboardMarkup(
    keyboard=[
        [KeyboardButton(text="Непонятен следующий шаг")],
        [KeyboardButton(text="Скучно / нет быстрой отдачи")],
        [KeyboardButton(text="Перегруз")],
...
kb_completed_day_open = ReplyKeyboardMarkup(
    keyboard=[
...
kb_minimal_feedback_done = ReplyKeyboardMarkup(
    keyboard=[[KeyboardButton(text="Да")], [KeyboardButton(text="Частично")], [KeyboardButton(text="Нет")]],
    resize_keyboard=True,
)

kb_minimal_feedback_help = ReplyKeyboardMarkup(
...
kb_skill_obstacle = ReplyKeyboardMarkup(
    keyboard=[
        ...
        [KeyboardButton(text="Стало тревожно")],
        [KeyboardButton(text="Не понял, что делать")],
        [KeyboardButton(text="Отвлёкся")],
        [KeyboardButton(text="Задача уже не актуальна")],
        [KeyboardButton(text="Другая причина")],
    ],
    resize_keyboard=True,
)

kb_post_action_reflection = ReplyKeyboardMarkup(
    keyboard=[
...
kb_experiment_completed = ReplyKeyboardMarkup(
    keyboard=[
        [KeyboardButton(text="Продолжить задачу самому")],
        [KeyboardButton(text="Ещё один маленький шаг")],
        [KeyboardButton(text="Закрыть день")],
...
kb_simplified_skill_after_effect = ReplyKeyboardMarkup(
    keyboard=[
        [KeyboardButton(text="🧭 Следующий шаг")],
        [KeyboardButton(text="🔄 Сменить навык")],
        [KeyboardButton(text="🌙 Закрыть день")],
...
def offer_recently_limited(u: Dict[str, Any], profile: Optional[Dict[str, Any]] = None, *, now: Optional[dt.datetime] = None) -> bool:
    ...
    for raw in (profile.get("offer_suppressed_until"), u.get("offer_suppressed_until")):
        ...
        if until and until > now:
            return True
    shown = _parse_aware_datetime(profile.get("offer_seen_at") or u.get("last_offer_shown_at"))
    return bool(shown and (now - shown) < dt.timedelta(days=7))
...
def scheduled_offer_due(u: Dict[str, Any], profile: Optional[Dict[str, Any]] = None, *, now: Optional[dt.datetime] = None) -> bool:
    ...
    for raw in (profile.get("offer_suppressed_until"), u.get("offer_suppressed_until")):
        ...
        if until and until > now:
            return False
    if int(u.get("day") or u.get("current_day") or 1) == 3 and day_closed_today(u, profile):
        return not offer_shown_today(u)
    shown = _parse_aware_datetime(profile.get("offer_seen_at") or u.get("last_offer_shown_at"))
    last_active = float(u.get("last_active") or 0)
    active_recently = last_active > 0 and now.timestamp() - last_active <= dt.timedelta(days=7).total_seconds()
    return bool(shown and now - shown >= dt.timedelta(days=7) and active_recently)


def has_unfinished_exercise_for_offer(u: Dict[str, Any]) -> bool:
    stage = str(u.get("stage") or "")
    if "crisis" in stage or stage in {"stuck_micro_action", "stuck_effect_await", "downscale_action", "consolidation_running", "extra_microstep"}:
        return True
    return str(u.get("current_state") or "") == STATE_AWAITING_RESULT
...
def can_show_offer(u: Dict[str, Any], profile: Optional[Dict[str, Any]] = None) -> bool:
    ...
    day1_completion_signal = bool(
        ...
        and int(working_model.get("evidence_count") or 0) >= 1
        and profile.get("personalized_insight_exists")
    )
    proof = ValueProof(
        completed_experiments=int(profile.get("completed_experiments") or 0),
        successful_or_partial=int(profile.get("successful_or_partial") or 0),
        personalized_insight_exists=bool(
            profile.get("personalized_insight_exists") or profile.get("main_hypothesis")
            or profile.get("main_pattern") or profile.get("barriers")
        ),
        user_has_seen_value_report=bool(profile.get("value_report_seen_at")),
        safety_active=bool(
        ...
        confirmed_working_skill_exists=bool(
            ...
            or profile.get("day1_offer_eligible")
            or day1_completion_signal
        ),
    )
    return (
        evaluate_value_proof(proof).eligible
        and not has_unfinished_exercise_for_offer(u)
        and not offer_recently_limited(u, profile)
    )
...
async def open_trainer_switch(m: Message, u: Dict[str, Any], source: str):
    ...
    set_legacy_stage(u, "trainer_switch")
    await save_user(u, DB_PATH)
...
async def return_after_trainer_switch(m: Message, u: Dict[str, Any], return_stage: str, switched: bool = False):
    set_legacy_stage(u, return_stage)
...
def trainer_general_line_for_user(u: Optional[Dict[str, Any]]) -> str:
    if not u:
...
def trainer_wrap(u: Dict[str, Any], text: str, scenario: str = "general") -> str:
    line = trainer_general_line_for_user(u) if scenario == "general" else trainer_style_line((u or {}).get('trainer_key') or 'marsha', scenario)
    return f"{line}\n\n{text}"


def is_skinny(u: Optional[Dict[str, Any]]) -> bool:
    return (u or {}).get("trainer_key") == "skinny"


def trainer_not_tried_text(u: Dict[str, Any]) -> str:
    if is_skinny(u):
        return "Стопор зафиксирован. Не обсуждаем его бесконечно. Уменьшаем шаг."
    return "Нормально. Это не провал и не оценка тебя."


def trainer_post_minimum_text(u: Optional[Dict[str, Any]]) -> str:
    if is_skinny(u):
        return (
            "Минимум выполнен. День уже не слит.\n\n"
            "Выбирай: закрепить ещё 2 минуты или закрыть тренировку.\n"
            "Можем сделать ещё один короткий подход. Не обязателен."
        )
    return POST_MINIMUM_CONTINUE_TEXT


def trainer_repeat_limit_text(u: Dict[str, Any]) -> str:
    if is_skinny(u):
        return (
            "Минимум выполнен. День уже не слит.\n\n"
            "Можем сделать ещё один короткий подход. Не обязателен."
        )
    return SUCCESS_REPEAT_LIMIT_TEXT


def trainer_skill_learning_reframe_text(u: Optional[Dict[str, Any]]) -> str:
    if is_skinny(u):
        return "Этот шаг оказался слишком большим. Хорошо. Теперь знаем размер следующего."
    return SKILL_LEARNING_REFRAME_TEXT
...
def combined_crisis_tool_text(patterns: List[str]) -> str:
    ...
    if len([p for p in patterns if p != "unknown"]) >= 3:
        labels = {
            "task_entry_block": "сломан вход в задачу",
            "attention_escape": "тянет в быстрый стимул",
            "perfectionism": "страшно ошибиться",
            "overwhelm": "задача слишком большая",
            "low_energy": "мало сил",
            "self_attack": "включился внутренний критик",
            "anxiety_loop": "есть тревога",
            "social_pain": "нужна опора и контакт",
        }
        selected_lines = [f"— {labels[p]};" for p in patterns if p in labels]
        summary = "\n".join(selected_lines[:6])
        anxiety_block = (
            "Тревогу сейчас не спорим и не доказываем ей, что всё нормально. "
            "Сначала снижаем уровень возбуждения тела.\n\n"
        ) if "anxiety_loop" in patterns else ""
        body_action = (
            "Длинный выдох; почувствовать опору ног; назвать 3 предмета вокруг; потом только один микрошаг."
            if "anxiety_loop" in patterns
            else "Сесть, сделать длинный выдох, выпить воды."
        )
        return (
            "Похоже, сейчас одновременно:\n"
            f"{summary}\n\n"
            "Значит, сейчас не решаем задачу. Сначала возвращаем опору, потом выбираем один микрошаг.\n\n"
            f"{anxiety_block}"
            "1. Тело:\n"
            f"{body_action}\n\n"
            "2. Фраза против критика:\n"
            "“Я застрял, но я не обязан себя добивать.”\n\n"
            "3. Один микрошаг:\n"
            "Открыть задачу / назвать первый шаг / написать одно слово — только после телесного сброса.\n\n"
            "Минимум:\n"
            "сесть + один длинный выдох.\n\n"
            "Сделай минимум и отметь результат."
        )
    parts = [crisis_tool_text(p) for p in patterns[:2] if p != "unknown"]
...
async def show_crisis_entry(m: Message, u: Dict[str, Any], source: str):
    set_legacy_stage(u, CRISIS_WAITING_INPUT)
...
async def show_crisis_tool_prompt(m: Message, u: Dict[str, Any]):
    set_legacy_stage(u, "crisis_tool_select")
    set_current_state(u, STATE_TASK_CRISIS_INPUT, close_action=True)
...
async def send_crisis_tool(m: Message, u: Dict[str, Any], reason_text: str):
    ...
    set_legacy_stage(u, "crisis_effect_await")
    ...
    if pattern == "high_risk":
        ...
        await m.answer(crisis_tool_text(crisis_stack))
    set_legacy_stage(u, "crisis_action_await")
    u["current_action_context"] = "crisis_stabilization"
    await save_user(u, DB_PATH)
    await answer_with_keyboard(m, u, "Сделай минимум и отметь результат.", kb_crisis_action, "crisis_action")
...
def apply_engine_updates(u: Dict[str, Any], screen: Dict[str, Any]):
    ...
    if screen.get("next_state"):
        set_legacy_stage(u, screen["next_state"])
...
def known_reply_button_texts() -> set[str]:
    keyboard_names = (
        ...
        "kb_completed_day_open", "kb_post_action_reflection", "kb_trainer_switch",
...
def button_fits_current_state(text: str, u: Dict[str, Any]) -> bool:
    ...
    by_stage = {
        ...
        "day_review_function": _reply_keyboard_texts(globals().get("kb_day_review_function")),
        "day_review_barrier": _reply_keyboard_texts(globals().get("kb_day_review_barrier")),
        "day_review_barrier_other": {"🌙 Закрыть без разбора"},
        "day_review_state": _reply_keyboard_texts(globals().get("kb_day_review_state")),
        "done": _reply_keyboard_texts(globals().get("kb_done")),
        "day_core_stop": _reply_keyboard_texts(globals().get("kb_day_core_stop")) | _reply_keyboard_texts(globals().get("kb_completed_day_open")),
        "closed_day_new_situation": _reply_keyboard_texts(globals().get("kb_completed_day_open")),
        ...
        "training": _reply_keyboard_texts(globals().get("kb_active_skill")) | ACTION_OUTCOME_BUTTONS,
        "new_day_skill": _reply_keyboard_texts(globals().get("kb_active_skill")) | ACTION_OUTCOME_BUTTONS,
        "current_skill": _reply_keyboard_texts(globals().get("kb_active_skill")) | ACTION_OUTCOME_BUTTONS,
        "downscale_action": _reply_keyboard_texts(globals().get("kb_active_skill")) | _reply_keyboard_texts(globals().get("kb_crisis_action")) | {"✅ Сделал", "🟡 Не вышло", "🟡 Попробовал, но не вышло", "↘️ Нужно проще", "🌙 Закрыть день"},
        "stuck_micro_action": _reply_keyboard_texts(globals().get("kb_crisis_action")),
        "stuck_effect_await": _reply_keyboard_texts(globals().get("kb_crisis_effect")),
        "stuck_aftercare": _reply_keyboard_texts(globals().get("kb_stuck_aftercare")),
        "minimal_feedback_done": _reply_keyboard_texts(globals().get("kb_minimal_feedback_done")),
        "minimal_feedback_help": _reply_keyboard_texts(globals().get("kb_minimal_feedback_help")),
        "minimal_feedback_next": _reply_keyboard_texts(globals().get("kb_minimal_feedback_next")),
        "skill_result_feedback": _reply_keyboard_texts(globals().get("kb_skill_result_feedback")),
        "skill_done_effect": _reply_keyboard_texts(globals().get("kb_skill_done_effect")),
        "skill_done_effect_text": set(),
        "skill_obstacle": _reply_keyboard_texts(globals().get("kb_skill_obstacle")),
        "skill_not_tried": _reply_keyboard_texts(globals().get("kb_skill_not_tried")),
        "extra_microstep": _reply_keyboard_texts(globals().get("kb_extra_microstep_done")),
    ...
    if stage in ACTION_RELATED_STAGES or stage in {"waiting_next_day", "training", "skill_card"}:
        allowed = set()
        for name in ("kb_training_main", "kb_more_actions", "kb_skill_card", "kb_new_day_skill", "kb_active_skill", "kb_failed", "kb_downscale", "kb_downscale_name_task", "kb_microstep", "kb_skeptic", "kb_done", "kb_crisis_action", "kb_crisis_effect", "kb_stuck_aftercare"):
...
def set_current_state(u: Dict[str, Any], state: str, *, new_action: bool = False, close_action: bool = False) -> str:
    ...
    if new_action:
        ...
        u["current_action_context"] = None
    return str(u.get("current_action_id") or "")


def normalize_action_context(u: Dict[str, Any]) -> str:
    """Backfill legacy sessions from the existing stage; never starts an action."""
    context = str(u.get("current_action_context") or "")
    if context:
        return context
    stage = str(u.get("stage") or "")
    if stage in {"crisis_stabilize", "crisis_action_await", "crisis_effect_await"}:
        context = "crisis_stabilization"
    elif stage in {"closed_day_voluntary_step", "closed_day_voluntary_tiny"}:
        context = "optional_after_close"
    elif (
        (u.get("current_action_id") and u.get("current_state") == STATE_AWAITING_RESULT)
        or stage in {"training", "downscale_action", "unclear_skill_simplified", "microstep", "downscale"}
        or str(u.get("daily_skill_status") or "") in {"in_progress", "stuck"}
    ):
        context = "experiment"
    if context:
        u["current_action_context"] = context
    return context
...
async def open_consolidation_branch(m: Message, u: Dict[str, Any], *, source: str = "consolidation") -> None:
    ...
    set_legacy_stage(u, "consolidation")
...
def mark_action_card_active(u: Dict[str, Any]) -> str:
    u["daily_skill_status"] = "in_progress"
    u["current_action_context"] = "experiment"
    ...
    u["active_attempt"] = {
        **default_active_attempt(u),
        ...
        "ranking_policy_version": "legacy",
        "post_experiment_action": None,
        "post_experiment_reason_code": None,
        "mastery_status": "NEW",
...
def should_reject_action_button(text: str, u: Dict[str, Any]) -> bool:
    if u.get("stage") in {
        "skill_change_reason", "skill_change_free_text", "skill_change_meaning",
        "stuck_validation_choice", "stuck_micro_action", "stuck_effect_await", "stuck_aftercare",
        "crisis_stabilize", "crisis_action_await", "crisis_effect_await",
        "closed_day_voluntary_step", "closed_day_voluntary_tiny", "extra_microstep",
    }:
        return False
...
async def show_action_changed_fallback(m: Message, u: Dict[str, Any], source: str = "action_context_mismatch"):
    ...
    set_legacy_stage(u, "experiment_completed_menu")
    set_current_state(u, STATE_PAUSED, close_action=True)
    await save_user(u, DB_PATH)
    await log_event(u.get("user_id"), u.get("stage", ""), "completed_attempt_button_ignored", {"source": source, "analytics_event": False}, DB_PATH, SHEETS_WEBHOOK_URL)
    await m.answer("Этот эксперимент уже отмечен как выполненный.", reply_markup=kb_experiment_completed)
...
def extra_microstep_prompt(u: Dict[str, Any]) -> str:
    return (
        "Ок. Две минуты без героизма.\n\n"
        "Сделай:\n"
        "1. Открой задачу.\n"
        "2. Поставь рядом точку входа: одно слово / один файл / один абзац.\n"
        "3. Побудь рядом 2 минуты.\n\n"
        "Минимум:\n"
        "открыть задачу на 10 секунд.\n\n"
        "После этого нажми:\n"
        "— ✅ Сделал\n"
        "— 🟡 Не вышло\n"
        "— 🌙 Закрыть день"
...
async def ask_instruction_clarity_feedback(m: Message, u: Dict[str, Any]) -> bool:
    ...
    if not await can_show_feedback_question(u, "feedback_instruction_clarity"):
        return False
    set_legacy_stage(u, "feedback_instruction_clarity")
    await save_user(u, DB_PATH)
...
async def ask_validation_feedback(m: Message, u: Dict[str, Any], reason: str, bot_answer: str) -> bool:
    if not await can_show_feedback_question(u, "feedback_validation", once_per_test=True):
        return False
    set_legacy_stage(u, "feedback_validation")
    await save_user(u, DB_PATH)
...
async def ask_day_value_feedback(m: Message, u: Dict[str, Any]) -> bool:
    ...
    if not await can_show_feedback_question(u, "feedback_day_value"):
        return False
    set_legacy_stage(u, "feedback_day_value")
    await save_user(u, DB_PATH)
...
async def ask_product_value_feedback(m: Message, u: Dict[str, Any], *, before_offer: bool = False) -> bool:
    ...
    if not await can_show_feedback_question(u, "product_value_score", once_per_test=True):
        return False
    set_legacy_stage(u, "feedback_product_value_score")
    await save_user(u, DB_PATH)
...
async def handle_feedback_response(m: Message, u: Dict[str, Any], text: str) -> bool:
    ...
    if stage == "feedback_validation":
        ...
        if value in {"missed", "free_text"} and text in values:
            set_legacy_stage(u, "feedback_validation_free_text")
            await save_user(u, DB_PATH)
    ...
    if stage == "feedback_day_value":
        ...
        if value == "nothing_yet":
            set_legacy_stage(u, "feedback_day_none_reason")
            await save_user(u, DB_PATH)
    ...
    if stage == "feedback_product_value_score":
        ...
        set_legacy_stage(u, "feedback_product_value_reason")
        await save_user(u, DB_PATH)
        if score <= 6:
...
SKILL_DONE_EFFECT_BY_BUTTON = {
    ...
    "😐 Без разницы": ("done_no_relief", "no_change", 0),
...
SKILL_OBSTACLE_BY_BUTTON = {
    ...
    "😣 Слишком сложно": "too_hard",
    "Не было сил": "no_energy",
    "Стало тревожно": "anxiety",
    "Не понял, что делать": "unclear_instruction",
    "🧠 Застрял / не понял, что делать": "unclear_instruction",
    "Отвлёкся": "distracted",
...
def not_done_reason_text(code: str, u: Dict[str, Any]) -> str:
    ...
    if code == "no_energy":
        return "Понял. Это плохой момент, а не провал. Сделай 30–60 секунд поддержки: сядь удобнее, выдохни и только назови задачу одним словом."
    if code == "anxiety":
        return "Понял. Сначала снижаем тревогу и требования: один медленный выдох, расслабь плечи, затем открой задачу без обязательства продолжать."
    if code == "unclear_instruction":
        return "Понял. Не меняю навык. Переформулирую: сделай только первый физический жест рядом с задачей — открыть файл, поставить курсор или положить руку на предмет задачи."
    if code == "distracted":
        return "Понял. Работаем со средой: положи телефон вне руки экраном вниз на 2 минуты и оставь открытой только вкладку задачи."
...
async def handle_not_done_context_reason(m: Message, u: Dict[str, Any], text: str) -> bool:
    if text == "Другая причина":
        ...
        return True
    custom_reason = " ".join(text.split())[:180] if u.get("stage") == "skill_obstacle_other" else ""
    code = "other_context" if custom_reason else SKILL_OBSTACLE_BY_BUTTON.get(text)
    if not code:
        await answer_with_keyboard(m, u, "Что помешало больше всего?", post_action_reason_keyboard(u), "skill_obstacle")
        return True
    sid = current_skill_id(u)
    profile = await get_user_profile(u["user_id"], DB_PATH)
    failed_count = int(profile.get("action_failed_count") or 0) + 1
    patch = {
        "last_not_completed_reason": code,
        "last_not_completed_skill": sid,
        "last_not_completed_is_context": True,
    ...
    if code == "too_hard":
        ...
        await bot_record_action_event(u, "no_energy_reported", skill_id=sid, metadata={"source": "not_done_reason", "context_not_failure": True})
    elif code == "anxiety":
        patch.update({"emotional_trigger": "anxiety", "needs_regulation_first": True})
    elif code == "unclear_instruction":
        patch.update({"instruction_unclear_count": int(profile.get("instruction_unclear_count") or 0) + 1})
    elif code == "distracted":
        patch.update({"attention_pattern": "environment_distraction", "needs_environment_change": True})
        await bot_record_action_event(u, "slip_reported", skill_id=sid, metadata={"source": "not_done_reason", "context_not_failure": True})
    elif code == "task_irrelevant":
        patch.update({"task_irrelevant_count": int(profile.get("task_irrelevant_count") or 0) + 1})
...
def get_minimal_feedback(u: Dict[str, Any]) -> Dict[str, Any]:
    raw = u.get("pending_feedback_json")
    if isinstance(raw, str):
        try:
            raw = json.loads(raw)
        except Exception:
...
def effect_status_from_minimal_feedback(helpfulness: str, continued: Any) -> str:
    """Keep task continuation separate from attributed skill helpfulness."""
...
def build_user_post_action_reflection(
...
async def persist_minimal_skill_feedback(m: Message, u: Dict[str, Any]) -> bool:
    ...
    if not feedback:
        return False
    sid = str(feedback.get("skill_id") or current_skill_id(u) or "")
...
async def _process_normalized_feedback(u: Dict[str, Any], feedback: Dict[str, Any]) -> None:
    """Run the deterministic outcome boundary when this attempt owns an experiment."""
    attempt = active_attempt(u)
    experiment_id = int(attempt.get("behavioral_experiment_id") or 0)
    revision = attempt.get("behavioral_experiment_revision")
    if experiment_id <= 0 or revision is None:
        return
    skill_id = str(feedback.get("skill_id") or attempt.get("current_skill_id") or "")
    card = ACTIVE_FILE_SKILL_REGISTRY.get(skill_id) if ACTIVE_FILE_SKILL_REGISTRY is not None else SKILL_REGISTRY.get(skill_id)
    if card is None:
        return
    completed = bool(feedback.get("completed"))
    partial = bool(feedback.get("partial"))
    ...
    if emotional_change == "worse":
        try:
            expected_flow_revision = get_flow_state(int(u["user_id"])).revision
        except Exception:
            expected_flow_revision = None
    try:
        processed = await process_experiment_outcome(
            DB_PATH,
        ...
        attempt.update({
            "post_experiment_action": processed.decision.action.value,
            "post_experiment_reason_code": processed.decision.reason_code,
            "mastery_status": processed.learning.state.status,
            "scaffolding_level": processed.learning.state.scaffolding_level,
        })
        u["active_attempt"] = attempt
    except Exception as exc:
        await log_event(
            int(u["user_id"]), "personalization", "normalized_outcome_processing_failed",
            {"error_type": type(exc).__name__, "experiment_id": experiment_id, "skill_id": skill_id},
            DB_PATH, SHEETS_WEBHOOK_URL,
        )


async def ask_skill_result_feedback(m: Message, u: Dict[str, Any], *, source: str = "done") -> bool:
    return await ask_minimal_skill_feedback(m, u, source=source)
...
async def finalize_skill_result_feedback(m: Message, u: Dict[str, Any], text: str, result_status: str, effect: str, effect_rating: int, *, completed: bool, send_menu: bool = True) -> bool:
    ...
    if send_menu:
        u.pop("skill_done_effect_source", "")
        feedback = {
        ...
        await record_profile_signal(u["user_id"], "training", {
            ...
            "last_session_pattern": reflection.personal_pattern,
            "last_tested_principle": reflection.tested_principle,
        }, source="post_action_reflection_legacy")
        set_legacy_stage(u, "post_action_reflection")
        await save_user(u, DB_PATH)
        await answer_with_keyboard(
            m, u, reflection.render(), kb_post_action_reflection, "post_action_reflection",
...
async def handle_skill_result_feedback(m: Message, u: Dict[str, Any], text: str) -> bool:
    ...
    if stage == "minimal_feedback_done":
        if text not in {"Да", "Частично", "Нет"}:
            await answer_with_keyboard(m, u, "Получилось сделать?", kb_minimal_feedback_done, "minimal_feedback_done")
            return True
        feedback = get_minimal_feedback(u) or minimal_feedback_base(u, source=str(u.get("skill_done_effect_source") or "done"))
        ...
        if text == "Нет":
            ...
            await answer_with_keyboard(
                m, u,
                "Не сработало — значит, текущего входа было недостаточно. Что помешало больше всего?",
                post_action_reason_keyboard(u), "skill_obstacle",
            )
            return True
    ...
    if stage == "skill_result_feedback":
        ...
        if text == "⏳ Не успел / не пробовал":
            set_legacy_stage(u, "skill_not_tried")
            await save_user(u, DB_PATH)
            await answer_with_keyboard(m, u, f"{trainer_not_tried_text(u)}\n\nКогда вернёшься, начнём с того же минимума или подберём другой вход.", kb_skill_not_tried, "skill_not_tried")
            return True
        await answer_with_keyboard(m, u, "Выбери, что ближе:", kb_skill_result_feedback, "skill_result_feedback")
        return True
    if stage == "skill_done_effect":
        attempt = active_attempt(u)
        if attempt.get("is_closed") and attempt.get("effect_status") != "unknown":
            set_legacy_stage(u, "experiment_completed_menu")
            await save_user(u, DB_PATH)
            await m.answer("Этот эксперимент уже отмечен как выполненный.", reply_markup=kb_experiment_completed)
            return True
        if text in {"✍️ Свой вариант", "✍️ Написать свой вариант"}:
            set_legacy_stage(u, "skill_done_effect_text")
            await save_user(u, DB_PATH)
    ...
    if stage == "skill_not_tried":
        ...
        if text == "🔄 Выбрать другой вход":
            ...
            await answer_with_keyboard(m, u, "Окей. Этот вход сейчас не твой. Что легче попробовать вместо него?", await other_entry_keyboard_for_user_async(u), "skill_other_entry")
            return True
        if text in {"🌙 Закрыть день", "🌙 На сегодня достаточно"}:
...
async def send_success_menu(m: Message, u: Dict[str, Any], *, source: str = "done"):
    ...
    set_current_state(u, STATE_PAUSED, close_action=True)
    await save_user(u, DB_PATH)
    await answer_with_keyboard(m, u, trainer_wrap(u, trainer_post_minimum_text(u), "continue"), success_menu_keyboard(u), "success_menu")
...
async def send_success_limit_menu(m: Message, u: Dict[str, Any]):
    ...
    set_current_state(u, STATE_PAUSED, close_action=True)
    await save_user(u, DB_PATH)
    await answer_with_keyboard(m, u, trainer_wrap(u, trainer_repeat_limit_text(u), "continue"), kb_success_limit, "success_limit")

STUCK_REASON_PROMPT = (
    "Стопор зафиксирован. Это не провал.\n"
    "Сейчас не надо заставлять себя сильнее — найдём, где именно ломается вход."
)
...
STUCK_REASON_CONFIG = {
    ...
    "shame": {
        "buttons": {"😬 Страшно ошибиться / стыдно", "😬 Страшно, стыдно, боюсь ошибиться", "😬 Страх ошибки / оценки"},
    ...
    "overwhelm": {
        "buttons": {"🧱 Слишком много задач", "🧠 Слишком много всего", "😣 Слишком сложно", "🌀 Слишком много вариантов", "😶 Не понимаю, с чего начать"},
    ...
    "anxiety": {
        "buttons": {"😰 Тревога или внутреннее напряжение", "🫨 Тревога и перегруз"},
        "skill_id": "body_before_task",
    ...
    "self_attack": {
        "buttons": {"🗯 Слишком сильно ругаю себя", "🧨 Самокритика после срыва"},
        "skill_id": "bad_first_step",
    ...
    "meaning": {
        "buttons": {"🫥 Не понимаю, зачем вообще делать", "🤷 Не понимаю, зачем это делать"},
        "skill_id": "visible_next_step",
...
def pending_stuck_micro_data(u: Dict[str, Any]) -> Dict[str, Any]:
    raw = u.get("pending_plan_change")
    if isinstance(raw, str) and raw:
        ...
        data = {}
    return data if isinstance(data, dict) and data.get("type") == "stuck_micro_action" else {}
...
async def send_repeated_simplification_body_reset(m: Message, u: Dict[str, Any], *, original_code: str, user_text: str = ""):
    ...
    mark_action_card_active(u)
    await save_user(u, DB_PATH)
    await answer_with_keyboard(m, u, trainer_wrap(u, f"{trainer_skill_learning_reframe_text(u)}\n\n{REPEATED_SIMPLIFICATION_BODY_TEXT}", "stuck"), action_keyboard(), "downscale")
...
async def start_stuck_text_validation(m: Message, u: Dict[str, Any], text: str):
    ...
    if stuck_text_needs_clarification(combined_text) and clarification_count < 3:
        ...
        set_legacy_stage(u, "stuck_reason_text")
...
async def start_analysis_clarification(m: Message, u: Dict[str, Any]) -> None:
    ...
    set_legacy_stage(u, "analysis_clarify_questions")
...
async def handle_analysis_clarification_answer(m: Message, u: Dict[str, Any], text: str) -> bool:
    ...
    u["pending_feedback_json"] = None
    set_legacy_stage(u, "analysis_clarify_done")
...
async def send_stuck_reason_skill(m: Message, u: Dict[str, Any], code: str, *, user_text: str = ""):
    ...
    mark_current_skill_status(u, "stuck")
    set_legacy_stage(u, "stuck_micro_action")
    u["last_simplification_modality"] = modality
    u["skill_variant_label"] = config["skill_name"]
    u["pending_stuck_reason"] = code
    u["pending_stuck_action_name"] = config["skill_name"]
    u["pending_stuck_skill_id"] = skill_id
    u["pending_stuck_micro_step"] = _clean_skill_line(config.get("minimum_step") or "один маленький вход")
    u["pending_plan_change"] = json.dumps(
        {
            "type": "stuck_micro_action",
            "reason": code,
            "action_name": config["skill_name"],
            "skill_id": skill_id,
            "micro_step": u["pending_stuck_micro_step"],
        },
        ensure_ascii=False,
    )
    u["pending_skill_id"] = None
    ...
    mark_action_card_active(u)
    await save_user(u, DB_PATH)
    minimum = u["pending_stuck_micro_step"]
    bot_answer = (
        "Нужен только один микро-шаг.\n\n"
        f"{config['skill_name']}:\n"
        f"1. {minimum}\n\n"
        "Сделай только это и отметь результат."
    )
    set_pending_validation_feedback(u, code, bot_answer)
    await save_user(u, DB_PATH)
    await answer_with_keyboard(m, u, trainer_wrap(u, bot_answer, "stuck"), kb_crisis_action, "stuck_micro_action")
...
async def open_misunderstood_flow(m: Message, u: Dict[str, Any], source: str):
    set_legacy_stage(u, "misunderstood_reason")
...
async def open_skill_change_reason(m: Message, u: Dict[str, Any]):
    set_legacy_stage(u, "skill_change_reason")
    await save_user(u, DB_PATH)
    await answer_with_keyboard(
        m,
        u,
...
def short_offer_text() -> str:
    ...
    return (
        "Следующий шаг — ещё 7 дней проверять карту на реальных задачах:\n\n"
        + "\n".join(options)
        + "\n\nВыбери, как закрепить результат. Перед оплатой можно открыть подробное заключение и конкретный план. Другие форматы поддержки вынесены отдельно."
    )
...
async def show_day3_offer(m: Message, u: Dict[str, Any], source: str, *, mode: str = "auto"):
    ...
    set_legacy_stage(u, OFFER_PREVIEW_STAGE if is_preview else OFFER_MENU_STAGE)
    u["offer_mode"] = mode
    ...
    await log_event(u["user_id"], "offer", "offer_seen", {**offer_meta, "offer_mode": mode}, DB_PATH, SHEETS_WEBHOOK_URL)
    ...
    if is_auto:
        await record_behavioral_analytics_event(
...
def pop_offer_resume_flow(u: Dict[str, Any]) -> Dict[str, Any] | None:
    ...
    set_legacy_stage(u, previous.get("stage") or u.get("stage") or "training")
...
async def leave_offer_menu(message: Message, u: Dict[str, Any], source: str) -> None:
    ...
    if await resume_after_offer_if_needed(message, u):
        return
    set_legacy_stage(u, "training_main" if not day_closed_today(u) else "day_core_stop")
...
async def maybe_show_offer(m: Message, u: Dict[str, Any], source: str) -> bool:
    ...
    profile.update({
        ...
        "successful_or_partial": metrics["successful_or_partial"],
        "personalized_insight_exists": bool(
            profile.get("main_hypothesis") or profile.get("main_pattern") or profile.get("barriers")
        ),
    })
    if (
        int(u.get("day") or 1) == 1
        and day_closed_today(u)
        and metrics["successful_or_partial"] >= 1
        and profile["personalized_insight_exists"]
    ):
        profile["day1_offer_eligible"] = True
    pre_report = ValueProof(
        metrics["completed_experiments"], metrics["successful_or_partial"],
    ...
    if eligibility.reason_codes == ("VALUE_REPORT_NOT_SEEN",):
        latest = metrics.get("latest_experiment") or {}
        report = PersonalValueReport(
            barrier=_offer_main_obstacle(summary, profile),
            experiment_checked=str(latest.get("instruction_variant") or "короткий поведенческий эксперимент"),
            observed_change=(
                "получилось выполнить критерий" if latest.get("success_criterion_met")
                else "получилось приблизиться к действию частично"
            ),
            skill_to_consolidate=day3_first_working_entry(summary, profile),
        )
        seen_at = dt.datetime.now(dt.timezone.utc).isoformat()
        await m.answer(trainer_wrap(u, render_personal_value_report(report), "map"))
        await update_user_profile(
            u["user_id"], {
                "completed_experiments": metrics["completed_experiments"],
                "successful_or_partial": metrics["successful_or_partial"],
                "personalized_insight_exists": profile["personalized_insight_exists"],
                "value_report_seen_at": seen_at,
            }, DB_PATH,
            source="value_report_seen_before_offer",
        )
        profile["value_report_seen_at"] = seen_at
        await record_behavioral_analytics_event(
            DB_PATH, BehavioralAnalyticsEvent("value_report_viewed", u["user_id"]), created_at=seen_at,
        )
    if not (scheduled_offer_due(u, profile) or should_show_day3_offer({**u, "profile_json": profile}, int(u.get("day") or 1))):
        await log_event(u["user_id"], u.get("stage", ""), "offer_blocked_by_guard", {
            "source": source, "day": int(u.get("day") or 1),
            "completed_experiments": metrics["completed_experiments"],
            "successful_or_partial": metrics["successful_or_partial"],
            "offer_shown": offer_was_shown(u, profile),
        }, DB_PATH, SHEETS_WEBHOOK_URL)
        if "NO_MEASURED_BENEFIT_YET" in eligibility.reason_codes:
            await m.answer(render_no_value_review())
        return False
...
OFFER_CALLBACKS = {
    ...
    "payment_claim": "offer:payment_claim",
    "back": "offer:back",
...
async def grant_paid_access(u: Dict[str, Any], source: str, meta: Optional[Dict[str, Any]] = None) -> None:
    ...
    await log_event(u["user_id"], u.get("stage", ""), "subscription_started", {"source": source, **meta}, DB_PATH, SHEETS_WEBHOOK_URL)
    if source not in {"test_payment_confirm", "admin_mark_paid"}:
        await record_behavioral_analytics_event(
            DB_PATH, BehavioralAnalyticsEvent("purchase_confirmed", u["user_id"]), created_at=now_iso,
        )
...
def offer_request_form_text(format_label: str, *, has_telegram_contact: bool = False) -> str:
    contact_line = (
        "Контакт уже есть в Telegram — повторять username или почту не нужно.\n"
        if has_telegram_contact else
        "Добавь один контакт для ответа — username, почту или телефон.\n"
    )
    return (
        ...
        "2. Что хочешь разобрать / какой запрос\n\n"
        "Можно коротко; Telegram-ник повторять не нужно."
    )
...
def lead_metadata(u: Dict[str, Any], from_user: Any, request_text: str, offer_type: str) -> Dict[str, Any]:
    username = getattr(from_user, "username", None) or u.get("username") or ""
    return {
        "telegram_user_id": u["user_id"],
        "telegram_username": username,
        "name": u.get("name") or getattr(from_user, "first_name", None) or "",
        "contact": f"@{username}" if username else "provided_in_request",
        "request": request_text[:1200],
        "source": "skiller",
...
def tariff_bot_text() -> str:
    return (
        ...
        "— история попыток и выводов;\n"
        "— следующий тест по результатам, а не случайный совет;\n"
        "— персональные напоминания — не больше двух в день;\n"
        "— журнал экспериментов и поведенческих цепочек;\n"
...
def tariff_bot_inline_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    if pay_url:
        ...
        rows.append([InlineKeyboardButton(text="✅ Уже оплатил(а) — проверить", callback_data=OFFER_CALLBACKS["payment_claim"])])
    else:
...
def sync_calendar_day(u: Dict[str, Any]) -> int:
    ...
    if has_stale_day_core_lock(u):
        clear_day_core_lock(u)
    reset_daily_state_for_local_date(u)
    return computed_day


def reset_daily_state_for_local_date(u: Dict[str, Any]) -> bool:
    """Drop expired, day-scoped routing locks without touching learned data."""
    today = local_date_for_user(u)
    previous = str(u.get("day_date") or u.get("last_day_closed_at") or "")[:10]
    if not previous:
        u["day_date"] = today
        return False
    if previous == today:
    ...
    # An old action must not make today's buttons complete yesterday's attempt.
    u["current_action_id"] = None
    u["active_attempt"] = None
    if str(u.get("stage") or "") in ACTIVE_SKILL_STAGES | ACTIVE_CRISIS_STAGES | {"awaiting_barrier_choice", "analysis_need_more", "day_core_stop"}:
        set_legacy_stage(u, "waiting_next_day")
    return True


def prepare_calendar_routing(u: Dict[str, Any]) -> None:
    reset_daily_state_for_local_date(u)
...
def early_personal_bundle_attempts(profile: Dict[str, Any]) -> int:
    """Count real early attempts enough to switch from generic advice to a personal bundle."""
    profile = profile or {}
    summary = profile.get("latest_summary") if isinstance(profile.get("latest_summary"), dict) else {}
    return max(
        int(profile.get("action_done_count") or profile.get("done_count") or 0),
        int(profile.get("attempts_started") or 0),
        int(profile.get("skill_attempts_total") or 0),
        int(profile.get("return_count") or 0),
        int(summary.get("done_count") or 0),
        len(_profile_list(profile.get("successful_skills"))),
        len(_profile_list(profile.get("failed_skills"))),
        len(_profile_list(profile.get("completed_skills_effect_unknown"))),
...
def has_early_personal_bundle_data(profile: Dict[str, Any]) -> bool:
    return early_personal_bundle_attempts(profile) >= 2


def early_personal_bundle_signals(profile: Dict[str, Any]) -> List[str]:
    profile = profile or {}
    signals: List[str] = []
    dev_map = profile.get("development_map") if isinstance(profile.get("development_map"), dict) else {}
    helps_text = " ".join(str(x).lower() for x in _profile_list(dev_map.get("helps")) + _profile_list(profile.get("working_strategies")))
    if int(profile.get("downscale_count") or 0) > 0 or any(token in helps_text for token in ("малень", "уменьш", "один", "коротк")):
        signals.append("задача уменьшается до одного действия")
    if profile.get("attention_pattern") == "scroll_autopilot" or int(profile.get("attention_escape_count") or 0) > 0:
        signals.append("телефон не лежит рядом")
    if (
        profile.get("avoidance_reason") == "fear_of_bad_result"
        or profile.get("main_pattern") in {"anxiety_avoidance", "shame_self_attack"}
        or profile.get("emotional_trigger") == "shame_or_anxiety"
        or profile.get("shame_signal")
    ):
        signals.append("можно сделать черновик без оценки")
    fallback = [
        "задача уменьшается до одного действия",
        "телефон не лежит рядом",
        "можно сделать черновик без оценки",
    ]
    for item in fallback:
        if len(signals) >= 3:
            break
...
def early_personal_bundle_intro(profile: Dict[str, Any]) -> str:
    signal_lines = "\n".join(f"— {item};" for item in early_personal_bundle_signals(profile))
    return (
        "Вчера появились первые сигналы.\n"
        "Тебе легче начинать, когда:\n"
        f"{signal_lines}\n\n"
        "Сегодня проверим не новый совет, а твою рабочую связку."
    )


def early_personal_bundle_route_text(day: int, profile: Dict[str, Any]) -> str:
    day = int(day or 0)
    if day == 4:
        return (
            "Связка на сегодня:\n"
            "Вчера вход получился.\n"
            "Сегодня добавим удержание:\n"
            "1. Телефон вне руки.\n"
            "2. Плохой черновик.\n"
            "3. 3 минуты рядом с задачей."
        )
    if day >= 5:
        return (
            "У тебя уже есть личная схема возврата:\n"
            "— убрать телефон;\n"
            "— не требовать качества;\n"
            "— начать с одной сырой строки;\n"
            "— оставить видимый следующий шаг.\n\n"
            "Сегодня проверим, что мешает вернуться после отвлечения."
        )
    return (
        "Связка на сегодня:\n"
        "1. Телефон вне руки на 3 минуты.\n"
        "2. Открыть задачу без требования работать.\n"
        "3. Написать один плохой черновик.\n\n"
        "Цель не сделать идеально.\n"
        "Цель — войти без бегства."
    )


def new_day_context_header(profile: Dict[str, Any]) -> str:
    if has_early_personal_bundle_data(profile):
        return f"🌱 Новый день.\n\n{early_personal_bundle_intro(profile)}\n"
...
def new_day_context_message(profile: Dict[str, Any]) -> str:
    if has_early_personal_bundle_data(profile):
        return f"{new_day_context_header(profile)}"
    return (
...
def new_day_skill_text(skill: Dict[str, Any], profile: Dict[str, Any], u: Optional[Dict[str, Any]] = None) -> str:
    day = int((u or {}).get("day") or 0)
    if 3 <= day <= 5 and has_early_personal_bundle_data(profile):
        return (
...
def _select_daily_skill_with_ranking(
    ...
    failed = set(_profile_list(profile.get("failed_skills"))) | set(blocked_today)
    current = str(current_skill_for_action(u) or "")
    states = {
        skill.id: PersonalSkillState(
            skill.id,
            mastery_status="PRACTICING" if skill.id in completed else "NEW",
            effectiveness_band="working" if skill.id in completed else ("unreliable" if skill.id in failed else "unknown"),
    ...
    try:
        ...
        return None
    selected_card = next(item for item in candidates if item.id == decision.selected_skill_id)
    skill = dict(SKILLS_DB.get(decision.selected_skill_id) or {
        "name": selected_card.title_user,
        "why_short": "Этот навык соответствует текущему механизму и контексту.",
        "how": selected_card.standard_variant,
        "simple": [selected_card.standard_variant],
        "minimum": selected_card.min_variant,
    })
    skill.update({
        "skill_id": decision.selected_skill_id,
        "id": decision.selected_skill_id,
        "selected_difficulty": decision.selected_difficulty,
        "progression_type": decision.progression_type,
        "ranking_reason_codes": list(decision.reason_codes),
        "ranking_policy_version": decision.policy_version,
    })
    skill.setdefault("variant", skill.get("variant") or decision.selected_skill_id)
    return skill
...
async def _start_normalized_experiment_for_day(u: Dict[str, Any], skill: Dict[str, Any]) -> None:
    """Create the evidence chain for feature-gated users without affecting legacy delivery."""
    if not (
        product_config.LEARNING_ENGINE_ENABLED and product_config.RANKING_ENGINE_ENABLED
        and product_config.use_new_architecture(int(u.get("user_id") or 0), is_test_user=bool(u.get("test_access")))
    ):
        return
    attempt = active_attempt(u)
    ...
    if card is None:
        return
    raw_mechanism = str(attempt.get("current_mechanism") or attempt.get("last_user_mechanism") or "")
    mechanism = legacy_mechanism_code(raw_mechanism) or card.mechanisms[0]
    context = str(attempt.get("context_domain") or u.get("current_task_context") or "other")
    ...
    try:
        ...
        parent_id = None
        if progression != "first":
            async with aiosqlite.connect(DB_PATH) as db:
                parent = await (await db.execute(
                    """SELECT id FROM behavioral_experiments
                       WHERE user_id=? AND skill_id=? AND status IN ('completed','safety_stopped')
                       ORDER BY id DESC LIMIT 1""",
                    (int(u["user_id"]), skill_id),
                )).fetchone()
            parent_id = int(parent[0]) if parent else None
            if parent_id is None:
                progression = "first"
        situation_id = await create_situation_snapshot(DB_PATH, SituationSnapshot(
            None, int(u["user_id"]), None, task, target, context, "start",
            50, 50, "today",
        ))
        mechanism_id = await create_mechanism_hypothesis(DB_PATH, MechanismHypothesis(
            None, situation_id, mechanism, "medium", ("user_started_feature_gated_day",),
            ("проверяем результат одного действия",), (), "rules", True,
        ))
        experiment_id = await create_behavioral_experiment(DB_PATH, BehavioralExperiment(
            None, int(u["user_id"]), situation_id, skill_id, mechanism, context,
            int(skill.get("selected_difficulty") or 1), card.standard_variant, target,
            card.completion_criterion, None, None, "proposed", parent_id,
            progression,
            (skill.get("ranking_reason_codes") or ["FEATURE_GATED_RANKING"])[0],
            str(u.get("trainer_key") or "marsha"), 0,
        ), mechanism_hypothesis_id=mechanism_id)
        revision = await transition_behavioral_experiment(
            DB_PATH, experiment_id, status="accepted", expected_revision=0,
        )
        revision = await transition_behavioral_experiment(
            DB_PATH, experiment_id, status="started", expected_revision=revision,
        )
        attempt.update({
        ...
        await log_event(
            ...
            {"error_type": type(exc).__name__, "skill_id": skill_id}, DB_PATH, SHEETS_WEBHOOK_URL,
        )
...
async def open_new_day_skill(m: Message, u: Dict[str, Any], day: int, source: str):
    ...
    u["active_attempt"] = {
        **default_active_attempt(u),
        ...
        "selected_difficulty": int(skill.get("selected_difficulty") or 1),
        "progression_type": skill.get("progression_type") or "first",
        "ranking_reason_codes": list(skill.get("ranking_reason_codes") or ()),
        "ranking_policy_version": skill.get("ranking_policy_version") or "legacy",
        "attempt_status": "not_tried",
        "effect_status": "unknown",
        "is_closed": False,
        "day_closed": False,
    }
    await _start_normalized_experiment_for_day(u, skill)
    if not u.get("current_task_title"):
        u["today_target"] = None
    ...
    if not profile.get("first_experiment_started_at"):
        started_at = dt.datetime.now(dt.timezone.utc).isoformat()
        await update_user_profile(u["user_id"], {"first_experiment_started_at": started_at}, DB_PATH, source="first_experiment_started")
        await log_event(u["user_id"], "training", "first_experiment_started", {"skill_id": sid}, DB_PATH, SHEETS_WEBHOOK_URL)
...
def should_skip_reactivation_for_closed_day(u: Dict[str, Any], today: str) -> bool:
    """Return True when today's closed day must be excluded from reactivation scheduling."""
    return closed_day_status(u) and closed_on_local_date(u, today)
...
def day_closed_action_text(u: Dict[str, Any]) -> str:
    return DAY_CLOSED_ACTION_TEXT
...
async def start_closed_day_additional_analysis(m: Message, u: Dict[str, Any], text: str) -> None:
    ...
    await save_user(u, DB_PATH)
    await log_event(
        u["user_id"], "day_completed", "additional_situation_started",
        {"source": "substantive_user_message", "daily_training_completed": True},
        DB_PATH, SHEETS_WEBHOOK_URL,
    )
...
async def handle_closed_day_input(m: Message, u: Dict[str, Any], text: str, low: str) -> bool:
    ...
    if u.get("stage") == "closed_day_new_situation":
        if closed_day_substantive_message(text):
            ...
            return True
        await answer_with_keyboard(
            m, u, "Опиши ситуацию одним сообщением — разберём её, не открывая заново основную тренировку.",
            kb_completed_day_open, "closed_day_new_situation",
        )
        return True
    ...
    if kind == "crisis":
        ...
        return True
    if text == "🎯 Разобрать ещё одну ситуацию":
        set_legacy_stage(u, "closed_day_new_situation")
        await save_user(u, DB_PATH)
        await m.answer("Опиши новую ситуацию одним сообщением или голосом.")
        return True
    if text == "⚡ Дать короткий навык":
        await open_closed_day_voluntary_step(m, u)
        return True
    ...
    if text == "🌙 На сегодня хватит":
        set_legacy_stage(u, "day_core_stop")
        await save_user(u, DB_PATH)
        await answer_with_keyboard(m, u, "Хорошо. Основная тренировка завершена; вернуться с новой ситуацией всё равно можно в любой момент.", kb_completed_day_open, "day_core_stop")
        return True
    if kind == "tomorrow":
        set_legacy_stage(u, "day_core_stop")
        await save_user(u, DB_PATH)
    ...
    if kind in DAY_CLOSED_BLOCKED_KINDS:
        set_legacy_stage(u, "day_core_stop")
        await save_user(u, DB_PATH)
        await answer_with_keyboard(m, u, day_closed_action_text(u), kb_completed_day_open, "day_core_stop")
        return True
    # Daily training is complete, but interaction is never locked.
    set_legacy_stage(u, "day_core_stop")
    await save_user(u, DB_PATH)
    await answer_with_keyboard(m, u, DAY_ALREADY_CLOSED_TEXT, kb_completed_day_open, "day_core_stop")
    return True

async def get_honest_day_counts(u: Dict[str, Any]) -> Dict[str, int]:
    day_id = str(u.get("current_day_id") or "")
    raw_counts = {
        "attempt_completed_self_reported": 0,
        "skill_result_reported": 0,
        "stuck_reason_selected": 0,
        "returned_after_slip": 0,
        "skill_changed": 0,
        "continued_target_task": 0,
    ...
    if day_id:
        async with aiosqlite.connect(DB_PATH) as db:
            cur = await db.execute(
                "SELECT event_type, metadata FROM action_events WHERE user_id=? AND day_id=? AND event_type IN ('attempt_completed_self_reported', 'skill_result_reported', 'stuck_reason_selected', 'returned_after_slip', 'skill_changed')",
                (u["user_id"], day_id),
            )
            for event_type, metadata in await cur.fetchall():
                event_type = str(event_type or "")
                raw_counts[event_type] = int(raw_counts.get(event_type) or 0) + 1
                if event_type != "skill_result_reported":
                    continue
                try:
                    meta = json.loads(metadata or "{}") if isinstance(metadata, str) else dict(metadata or {})
                except (TypeError, ValueError, json.JSONDecodeError):
                    meta = {}
                after_action = str(meta.get("after_action") or "")
                if after_action in {"continued_target_task", "stopped_after_step", "did_something_else"}:
                    raw_counts[after_action] += 1
    attempts = max(raw_counts["skill_result_reported"], raw_counts["attempt_completed_self_reported"])
    stopped = raw_counts["stuck_reason_selected"] + raw_counts["stopped_after_step"]
    return {
        "completed_actions_today": raw_counts["attempt_completed_self_reported"],
        "attempts_today": attempts,
        "continued_actions_today": raw_counts["continued_target_task"],
        "stopped_after_step_today": raw_counts["stopped_after_step"],
        "switched_after_step_today": raw_counts["did_something_else"],
        "stuck_events_today": stopped,
        "returns_today": raw_counts["returned_after_slip"],
        "skill_changes_today": raw_counts["skill_changed"],
    }


def _development_check_text(value: Any) -> str:
    """Render one model check without leaking a Python/JSON representation."""
    if isinstance(value, dict):
        label_text = public_enum_text(value.get("label") or value.get("title") or "")
        evidence = int(value.get("evidence_count") or value.get("evidence*count") or 0)
        contradictions = int(value.get("contradiction_count") or value.get("contradiction*count") or 0)
        if contradictions:
            status = "есть противоречивые данные"
        elif evidence >= 2:
            status = "есть повторный сигнал"
        elif evidence == 1:
            status = "есть один сигнал"
        else:
            status = "ещё проверяем"
        return f"{label_text}: {status}"
    return public_enum_text(value)


def model_confidence_text(skill_map: Dict[str, Any], attempts: int) -> str:
    """Conservative confidence: volume alone is not repeated evidence."""
    skills = list((skill_map or {}).get("skills") or [])
    repeated = max(
        (max(int(item.get("helpful_count") or 0), int(item.get("stuck_count") or 0)) for item in skills),
        default=0,
    )
    contradictory = any(
        int(item.get("helpful_count") or 0) > 0 and int(item.get("stuck_count") or 0) > 0
        for item in skills
    )
    if repeated >= 3 and attempts >= 4 and not contradictory:
        return "высокая"
    if repeated >= 2 or (attempts >= 3 and not contradictory):
        return "средняя"
    return "низкая"


def render_prelaunch_full_map(u: Dict[str, Any], profile: Dict[str, Any], skill_map: Dict[str, Any]) -> str:
    model = profile.get("personal_working_model") if isinstance(profile.get("personal_working_model"), dict) else {}
...
DAY_REVIEW_FUNCTION_LABELS = {
    "start": "START — вход в задачу",
    "stay": "STAY — удержание после первого шага",
    "return": "RETURN — возврат после отвлечения",
    "returned": "RETURN — возврат получился",
}

DAY_REVIEW_NEXT_TESTS = {
    "start": "сделать первый шаг конкретным и видимым",
    "stay": "после микрошагa остаться с одной задачей ещё 3 минуты",
    "return": "после отвлечения назвать «возврат» и открыть место задачи",
    "returned": "повторить тот же короткий протокол возврата ещё один раз",
}


def day_review_profile(profile: Dict[str, Any]) -> Dict[str, Any]:
    value = profile.get("last_day_review")
...
def prediction_check_keyboard(prediction_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(
        text=label, callback_data=f"prediction:{prediction_id}:{code}"
    )] for label, code in (("Да", "yes"), ("Частично", "partial"), ("Нет", "no"), ("Не заметил", "unknown"))])


async def maybe_show_day1_insight(m: Message, u: Dict[str, Any]) -> bool:
    if not ENABLE_DAY1_WOW or int(u.get("day") or 1) != 1:
        return False
    profile = await get_user_profile(u["user_id"], DB_PATH)
    model = profile.get("personal_working_model") if isinstance(profile.get("personal_working_model"), dict) else {}
    attempts = int(model.get("evidence_count") or 0)
    if attempts < 1 or attempts > 2 or profile.get("day1_insight_seen_at"):
        return False
    text, prediction = day1_insight_text(u, profile)
    seen_at = dt.datetime.now(dt.timezone.utc).isoformat()
    prediction_id = f"prediction_{uuid.uuid4().hex[:10]}"
    await update_user_profile(u["user_id"], {
        "day1_insight_seen_at": seen_at,
        "personalized_insight_exists": True,
        "day1_prediction": prediction,
        "day1_prediction_id": prediction_id,
        "day1_prediction_status": "UNTESTED",
    }, DB_PATH, source="day1_wow")
...
async def mark_day_closed(u: Dict[str, Any], source: str):
    ...
    if day_number == 1:
        await log_event(u["user_id"], "day1", "day1_completed", {"completed_actions": completed_actions_today}, DB_PATH, SHEETS_WEBHOOK_URL)
...
async def handle_reactivation_reply(m: Message, u: Dict[str, Any], text: str, low: str) -> bool:
    ...
    if text in {"⚡ Навык на 2 минуты", "Дать действие попроще", "Начать короткую тренировку"}:
        ...
        if not await restore_from_notification_context(m, u, source="reactivation_short_skill"):
    ...
    if await restore_from_notification_context(m, u, source="reactivation_continue"):
        return True
    stage = str(u.get("stage") or "")
    if stage in {"training", "await_training_target", "waiting_next_day", "confirm_analysis", "analysis_next_step"}:
        await handle_action_request(u["user_id"], m, u, repeat=True)
        return True
    await open_safe_day_menu_from_notification(m, u, "missing_reactivation_context")
    return True


def _event_meta(row: Dict[str, Any]) -> Dict[str, Any]:
    raw = row.get("event_data") or row.get("meta") or "{}"
    try:
...
def _action_meta(row: Dict[str, Any]) -> Dict[str, Any]:
...
def _local_day_from_iso(value: Any) -> str:
    return str(value or "")[:10]

async def build_owner_funnel_report(db_path: str, *, report_date: str = "") -> str:
    """Build a clean owner funnel from external, analytics-eligible product signals only."""
    async with aiosqlite.connect(db_path) as db:
        db.row_factory = aiosqlite.Row
        user_rows = [dict(r) for r in await (await db.execute("SELECT * FROM users")).fetchall()]
        event_rows = [dict(r) for r in await (await db.execute("SELECT * FROM events WHERE COALESCE(is_internal_test, 0)=0 AND COALESCE(analytics_event, 1)=1")).fetchall()]
        action_rows = [dict(r) for r in await (await db.execute("SELECT * FROM action_events")).fetchall()]

    internal_ids = set(INTERNAL_TEST_USER_IDS)
    external_users = {int(u.get("user_id") or 0): u for u in user_rows if int(u.get("is_test_user") or 0) != 1 and int(u.get("user_id") or 0) not in internal_ids}
    external_ids = set(external_users)

    if report_date:
        event_rows = [r for r in event_rows if _local_day_from_iso(r.get("created_at")) == report_date]
        action_rows = [r for r in action_rows if _local_day_from_iso(r.get("created_at")) == report_date]

    def clean_event(r: Dict[str, Any]) -> bool:
        name = str(r.get("event_name") or r.get("event") or "")
        if name in {"closed_day_automatic_message_blocked", "duplicate_callback_ignored", "stale_callback_ignored", "keyboard_shown", "keyboard_warning"}:
            return False
        meta = _event_meta(r)
        if meta.get("analytics_event") is False or meta.get("duplicate_product_metric") is True:
            return False
        return int(r.get("user_id") or 0) in external_ids

    def clean_action(r: Dict[str, Any]) -> bool:
        meta = _action_meta(r)
        if meta.get("analytics_event") is False or meta.get("duplicate_product_metric") is True or meta.get("is_internal_test") is True:
            return False
        return int(r.get("user_id") or 0) in external_ids

    events = [r for r in event_rows if clean_event(r)]
    actions = [r for r in action_rows if clean_action(r)]

    by_event: Dict[str, set[int]] = {}
    for r in events:
        by_event.setdefault(str(r.get("event_name") or r.get("event") or ""), set()).add(int(r.get("user_id") or 0))

    action_by_type: Dict[str, set[int]] = {}
    useful_days: Dict[int, set[str]] = {}
    helped_users: set[int] = set()
    continued_users: set[int] = set()
    completed_skill_users: set[int] = set()
    for r in actions:
        uid = int(r.get("user_id") or 0)
        et = str(r.get("event_type") or "")
        action_by_type.setdefault(et, set()).add(uid)
        meta = _action_meta(r)
        if et == "skill_result_reported":
            completed = meta.get("completed") is True or str(meta.get("result_status") or "").startswith("done")
            partial = meta.get("partial") is True
            continued = meta.get("continued_after_skill") is True
            helpful = str(meta.get("helpfulness") or meta.get("effect") or "") in {"helped", "some", "easier", "started_task"}
            if completed or partial:
                completed_skill_users.add(uid)
            if helpful:
                helped_users.add(uid)
            if continued:
                continued_users.add(uid)
            if completed or partial or continued:
                useful_days.setdefault(uid, set()).add(_local_day_from_iso(r.get("created_at")))
        elif et == "attempt_completed_self_reported":
            completed_skill_users.add(uid)
            useful_days.setdefault(uid, set()).add(_local_day_from_iso(r.get("created_at")))

    first_useful_day = {uid: min(days) for uid, days in useful_days.items() if days}
    d1_retained = {uid for uid, days in useful_days.items() if uid in first_useful_day and any(day > first_useful_day[uid] for day in days)}
    d3_completed = {uid for uid, days in useful_days.items() if len(days) >= 3}

    first_skill_users = by_event.get("new_day_skill_opened", set()) | by_event.get("current_skill_resent", set()) | action_by_type.get("attempt_started", set())
    ...
    lines = [
        ...
        "Только внешние пользователи, без технических дублей.",
        "",
        f"Новые уникальные пользователи: {len(by_event.get('start', set()))}",
        f"Завершили онбординг: {len(by_event.get('onboarding_completed', set()) | by_event.get('notifications_consent_set', set()))}",
        f"Завершили диагностику: {len(by_event.get('diagnosis_completed', set()))}",
        f"Получили анализ: {len(by_event.get('analysis_shown', set()) | by_event.get('recommended_track_shown', set()))}",
        f"Получили первый навык: {len(activated)}",
        f"Начали первый навык: {len(action_by_type.get('attempt_started', set()))}",
        f"Завершили первый навык: {len(completed_skill_users)}",
        f"Навык помог: {len(helped_users)}",
        f"Продолжили основную задачу: {len(continued_users)}",
        f"Закрыли день: {len(action_by_type.get('day_closed', set()) | by_event.get('day_closed', set()))}",
        f"Вернулись на следующий день: {len(d1_retained)}",
        f"Вернулись после реактивации: {len(returned_after_reactivation)}",
        f"Дошли до третьего дня: {len(d3_completed)}",
        f"Увидели оффер: {len(by_event.get('offer_shown', set()) | by_event.get('offer_auto_shown', set()) | by_event.get('offer_details_requested', set()))}",
        f"Нажали оплату: {len(payment_clicked)}",
        f"Оплатили: {len(paid)}",
    ]
    return "\n".join(lines)
...
async def handle_user_command(m: Message, u: Dict[str, Any], text: str) -> bool:
    ...
    if command in {"/owner_funnel", "/daily_funnel"}:
        if uid != CURATOR_TELEGRAM_ID and not qa_command_allowed(uid, u):
            await m.answer("Команда доступна владельцу.")
            return True
        parts = text.split(maxsplit=1)
        report_date = parts[1].strip()[:10] if len(parts) == 2 else ""
        await m.answer(await build_owner_funnel_report(DB_PATH, report_date=report_date))
        return True
...
async def handle_admin_command(m: Message, u: Dict[str, Any], text: str) -> bool:
    ...
    admin_button_commands = {
        "🧹 Reset my test user": "/reset_test_user", "🌅 Start new day": "/force_next_day",
        "🌙 Mark day closed": "/admin_close_day", "💳 Enable paid": "/grant_full",
        "💳 Disable paid": "/revoke_full", "🧠 Show state": "/debug_state",
        "🎯 Jump to Day 1 insight": "/admin_day1_insight", "🔮 Jump to prediction": "/admin_prediction",
        "💰 Show offer": "/show_offer", "🧪 Create fake successful experiment": "/admin_fake_success",
        "🧪 Create fake failed experiment": "/admin_fake_failure",
    }
    command = admin_button_commands.get(text, normalize_slash_command(text))
    qa_commands = {
    ...
    admin_only_commands = {
        ...
        "/simulate_paid", "/simulate_unpaid", "/sync_sheets", "/stats", "/admin",
        "/admin_close_day", "/admin_day1_insight", "/admin_prediction", "/admin_fake_success", "/admin_fake_failure",
    }
    ...
    if command == "/admin":
        keyboard = ReplyKeyboardMarkup(keyboard=[
            [KeyboardButton(text="🧹 Reset my test user"), KeyboardButton(text="🌅 Start new day")],
            [KeyboardButton(text="🌙 Mark day closed"), KeyboardButton(text="🧠 Show state")],
            [KeyboardButton(text="💳 Enable paid"), KeyboardButton(text="💳 Disable paid")],
            [KeyboardButton(text="🎯 Jump to Day 1 insight"), KeyboardButton(text="🔮 Jump to prediction")],
            [KeyboardButton(text="💰 Show offer")],
            [KeyboardButton(text="🧪 Create fake successful experiment")],
            [KeyboardButton(text="🧪 Create fake failed experiment")],
        ], resize_keyboard=True)
        await m.answer("Admin / staging controls", reply_markup=keyboard)
        return True

    if command == "/admin_close_day":
        await mark_day_closed(u, "admin_menu")
        await save_user(u, DB_PATH)
        await m.answer("Тестовый день закрыт.")
        return True

    if command in {"/admin_fake_success", "/admin_fake_failure"}:
        profile = await get_user_profile(uid, DB_PATH)
        successful = command.endswith("success")
        model = update_working_model(
            profile.get("personal_working_model"), barrier="перегруз",
            skill_title="Открыть без таймера", context="work", successful=successful,
            evidence_ref=f"admin:{uuid.uuid4().hex[:8]}", step_size="10 секунд",
        )
        await update_user_profile(uid, {
            "personal_working_model": model.as_dict(),
            "last_skill_completed": successful,
            "last_continued_after_skill": successful,
            "personalized_insight_exists": True,
        }, DB_PATH, source="admin_fake_experiment")
        await m.answer("Создан тестовый успешный эксперимент." if successful else "Создан тестовый неуспешный эксперимент.")
        return True

    if command == "/admin_day1_insight":
        profile = await get_user_profile(uid, DB_PATH)
        text_value, prediction = day1_insight_text(u, profile)
        await m.answer(text_value)
        await log_event(uid, "admin", "day1_wow_seen", {"source": "admin_jump"}, DB_PATH, SHEETS_WEBHOOK_URL)
        return True

    if command == "/admin_prediction":
        profile = await get_user_profile(uid, DB_PATH)
        prediction = str(profile.get("day1_prediction") or day1_insight_text(u, profile)[1])
        prediction_id = str(profile.get("day1_prediction_id") or f"prediction_{uuid.uuid4().hex[:10]}")
        await update_user_profile(uid, {"day1_prediction": prediction, "day1_prediction_id": prediction_id, "day1_prediction_status": "UNTESTED"}, DB_PATH, source="admin_prediction")
        await m.answer(f"🔮 {prediction}", reply_markup=prediction_check_keyboard(prediction_id))
        return True
...
async def ask_today_action(m: Message, u: Dict[str, Any]):
    ...
    if analysis_data.get("last_morning_state_date") != today and not day_core_test_mode_enabled(u):
        set_legacy_stage(u, "morning_checkin")
...
BARRIER_BUTTONS = {
    "😵 Перегруз": ("overload", "visible_next_step"),
    "😬 Страх ошибки": ("fear_of_error", "bad_first_step"),
    "📱 Отвлечения": ("attention_escape", "phone_far_3min"),
    "🌀 Слишком много вариантов": ("too_many_options", "visible_next_step"),
    "😶 Не вижу смысла": ("low_reward", "task_naming"),
}
...
def public_enum_text(value: Any) -> str:
    ...
    # Fail closed for an unrecognised machine token.  A human-safe generic
    # label is more honest than exposing an enum as product copy.
...
async def continue_analysis_from_barrier(m: Message, u: Dict[str, Any], text: str) -> None:
    """Resolve the low-evidence question from saved analysis, without an LLM call."""
    if text in BARRIER_BUTTONS:
        barrier, fallback_skill = BARRIER_BUTTONS[text]
    else:
        by_code = {code: skill for code, skill in BARRIER_BUTTONS.values()}
        barrier, fallback_skill = text, by_code[text]
    try:
        comp = json.loads(u.get("analysis_json") or "{}")
    except (TypeError, ValueError, json.JSONDecodeError):
        comp = {}
    if not isinstance(comp, dict):
        comp = {}
    skill_id = fallback_skill if fallback_skill in SKILLS_DB else "open_only"
    comp["analysis_id"] = comp.get("analysis_id") or f"analysis_{uuid.uuid4().hex[:12]}"
    comp["selected_barrier"] = barrier
    comp["barrier_candidates"] = list(dict.fromkeys([*(comp.get("barrier_candidates") or []), barrier]))
...
async def apply_conclusion_correction(m: Message, u: Dict[str, Any], correction: str) -> None:
    """Apply an explicit user correction to the saved conclusion, without rediagnosis."""
    try:
        comp = json.loads(u.get("analysis_json") or "{}")
    except (TypeError, ValueError):
        comp = {}
...
DAY_REVIEW_FUNCTION_BY_BUTTON = {
    "START — не начал": "start",
    "STAY — начал и остановился": "stay",
    "RETURN — отвлёкся и не вернулся": "return",
    "Вернулся и продолжил": "returned",
}

DAY_REVIEW_BARRIER_BY_BUTTON = {
    "Непонятен следующий шаг": "первое действие было непонятно",
    "Скучно / нет быстрой отдачи": "скука или отсутствие быстрой отдачи",
    "Перегруз": "перегруз",
    "Страх ошибки или оценки": "страх ошибки или оценки",
    "Телефон / YouTube": "телефон или YouTube перехватили внимание",
    "Усталость или сильные эмоции": "усталость или сильные эмоции",
}
...
DAY_REVIEW_STATE_BY_BUTTON = {
    ...
    "Состояние менялось": "состояние менялось",
}


def _day_review_data(u: Dict[str, Any]) -> Dict[str, Any]:
    raw = u.get("pending_feedback_json")
    if isinstance(raw, str):
        try:
            raw = json.loads(raw)
            ...
            raw = {}
    if not isinstance(raw, dict) or raw.get("kind") != "day_review":
        return {"kind": "day_review"}
    return dict(raw)


async def start_day_review(m: Message, u: Dict[str, Any], source: str) -> None:
...
async def finalize_day_review(m: Message, u: Dict[str, Any], review: Dict[str, Any], source: str) -> None:
    current_day = sync_calendar_day(u)
    review = {
        "function": str(review.get("function") or ""),
        "barrier": str(review.get("barrier") or ""),
    ...
    await save_user(u, DB_PATH)
    await log_event(
        u["user_id"], "training", "day_closed_with_review",
        {"day": current_day, "day_id": u.get("current_day_id"), **review},
        DB_PATH, SHEETS_WEBHOOK_URL,
    )
    profile = await get_user_profile(u["user_id"], DB_PATH)
...
async def handle_day_review(m: Message, u: Dict[str, Any], text: str) -> bool:
    ...
    if stage == "day_review_barrier":
        ...
        if not barrier:
            ...
            return True
        review["barrier"] = barrier
    elif stage == "day_review_barrier_other":
        value = " ".join(str(text or "").split())[:180]
        if len(value) < 3:
            await m.answer("Нужна одна короткая фраза: что мешало?")
            return True
...
async def close_day_from_global_button(m: Message, u: Dict[str, Any], source: str):
    if day_closed_today(u):
        ...
        await answer_with_keyboard(m, u, "На сегодня всё. День уже закрыт.", kb_day_core_stop, "day_core_stop")
        return
    await start_day_review(m, u, source)
...
async def handle_global_button(m: Message, u: Dict[str, Any], text: str) -> bool:
    ...
    if kind == "action":
        ...
        return True
    if kind == "enough":
        set_legacy_stage(u, "day_pause_confirm")
        set_current_state(u, STATE_PAUSED, close_action=True)
        await save_user(u, DB_PATH)
    ...
    if kind == "stuck":
        ...
        set_legacy_stage(u, "failed_options")
        set_current_state(u, STATE_AWAITING_STUCK_REASON)
        await save_user(u, DB_PATH)
    ...
    if kind == "downscale":
        ...
        await simplify_current_action(m, u)
        return True
    if kind == "more":
    ...
    if kind == "skip":
        ...
        set_legacy_stage(u, "skip_options")
...
def simplified_step(value: str) -> str:
    """Deterministic minimum-step reducer used by every action context."""
    text = " ".join(str(value or "").split())
    replacements = (
        (r"\b(?:30|60|90)\s*(?:секунд[уы]?|сек)\b", "10 секунд"),
        (r"\b3\s+(?:действия|шага|пункта)\b", "1 действие"),
        (r"написать (?:одну )?фразу", "написать 1–3 слова"),
        (r"открыть (?:задачу )?и начать", "только открыть"),
    )
    for pattern, replacement in replacements:
        text = re.sub(pattern, replacement, text, flags=re.IGNORECASE)
    return text or "Только открыть задачу на 10 секунд."


async def simplify_current_action(m: Message, u: Dict[str, Any]) -> None:
    sid = current_skill_for_action(u) or current_skill_id(u) or u.get("daily_skill_id")
    skill = dict(SKILLS_DB.get(sid) or {})
    minimum = skill.get("minimum_action") or skill.get("minimum") or skill.get("micro")
    current = str(u.get("current_next_physical_step") or "")
    smaller = simplified_step(str(minimum or current))
    u["current_next_physical_step"] = smaller
    u["current_action_context"] = u.get("current_action_context") or "experiment"
    set_legacy_stage(u, "downscale_action")
    sync_active_attempt(u, current_mechanism=active_attempt(u).get("current_mechanism"))
    await save_user(u, DB_PATH)
    note = "\n\nЭто уже минимум — после него можно сразу отметить результат." if smaller == current and current else ""
    await answer_with_keyboard(m, u, f"Версия проще:\n{smaller}{note}", action_keyboard(), "downscale_action")
...
DIAGNOSTIC_INPUT_STAGES = {
    "await_problem_text", "await_problem_voice", "run_analysis", "analysis_retry_await_clarification",
    "analysis_refine", "misunderstood_problem_await", "misunderstood_explain_await",
}
...
kb_existing_user_start = ReplyKeyboardMarkup(
    keyboard=[
        ...
        [KeyboardButton(text="Начать всё заново")],
    ],
    resize_keyboard=True,
)

kb_restart_confirm = ReplyKeyboardMarkup(
    keyboard=[
        [KeyboardButton(text="Да, начать всё заново")],
        [KeyboardButton(text="Нет, вернуться")],
    ],
    resize_keyboard=True,
)

def is_registered_user(u: Dict[str, Any]) -> bool:
    return bool(
        u.get("first_start_date")
        or u.get("name")
        or int(u.get("profile_completed") or 0) == 1
        or int(u.get("diagnostic_completed") or 0) == 1
        or int(u.get("has_started_training") or 0) == 1
        or u.get("current_day_id")
        or u.get("day_core_skill_date")
    )
...
async def handle_successful_payment(m: Message):
    ...
    await m.answer("✅ Оплата прошла. Полный режим включён автоматически.")
    if not await resume_after_offer_if_needed(m, u):
        await send_full_mode_welcome(m, u)
...
async def main_flow(m: Message):
    ...
    # closed-day or stale-action guard, including for returning users.
    prepare_calendar_routing(u)
    normalize_action_context(u)
    await save_user(u, DB_PATH)
    ...
    if text in BARRIER_BUTTONS:
        if u.get("stage") in {"analysis_need_more", "awaiting_barrier_choice"}:
            await continue_analysis_from_barrier(m, u, text)
        else:
            await answer_with_keyboard(m, u, "Этот экран уже не актуален.", kb_context_fallback, u.get("stage") or "training")
        return
    if text == "✏️ Исправить вывод":
        set_legacy_stage(u, "awaiting_conclusion_correction")
        await save_user(u, DB_PATH)
        await m.answer("Напиши или скажи одной короткой фразой, что в выводе нужно исправить.", reply_markup=ReplyKeyboardRemove())
        return
    if u.get("stage") == "awaiting_conclusion_correction":
        if text:
    ...
    if u.get("stage") == "existing_user_start_menu":
        ...
        if text in {"Начать новый день", "Сегодня другое"}:
            ...
            return
        if text == "Открыть мою карту":
            await log_event(uid, "start_menu_open_map", {"analytics_event": False}, db_path=DB_PATH)
            await send_user_map(m, u, "repeat_start_menu")
            return
        if text == "Начать всё заново":
            set_legacy_stage(u, "restart_onboarding_confirm")
            await save_user(u, DB_PATH)
    ...
    # The behavioural diary card owns its reply buttons and free-text
    # alternative until it is complete.  Route it before stale/global button
    # recovery so a close-day answer can never be interpreted as a new task.
    if await handle_day_review(m, u, text):
        return
    ...
    if u.get("stage") == "reminder_overload_settings":
        mode_by_text = {
            "Оставить одно в день": "one_per_day",
            "Только утром": "morning_only",
            "Только вечером": "evening_only",
            "Поставить на паузу": "paused",
        }
        if text in mode_by_text:
            u["reminder_mode"] = mode_by_text[text]
            if u["reminder_mode"] == "paused":
                u["notifications_enabled"] = 0
            await save_user(u, DB_PATH)
            await log_event(uid, "notifications", "reminder_mode_changed", {"mode": u["reminder_mode"], "analytics_event": False}, DB_PATH, SHEETS_WEBHOOK_URL)
            await answer_with_keyboard(m, u, "Ок. Настроил напоминания без преследования.", kb_day_menu, "day_menu")
            return
        await answer_with_keyboard(m, u, "Похоже, напоминания сейчас не помогают. Как поступить?", kb_reminder_overload, "reminder_overload_settings")
        return

    if u.get("stage") == "notification_active_action":
        if text == "Продолжить":
            await log_event(uid, "notification", "notification_continue_clicked", {"analytics_event": False}, DB_PATH, SHEETS_WEBHOOK_URL)
            await send_current_skill(uid, m, u)
            return
        if text == "Сделать проще":
            await log_event(uid, "notification", "notification_simplify_clicked", {"analytics_event": False}, DB_PATH, SHEETS_WEBHOOK_URL)
            await handle_action_request(uid, m, u, repeat=True)
            return
        if text == "Другой способ":
            await log_event(uid, "notification", "notification_other_way_clicked", {"analytics_event": False}, DB_PATH, SHEETS_WEBHOOK_URL)
            await handle_action_request(uid, m, u, repeat=True)
            return
        if text == "Больше не актуально":
            await log_event(uid, "notification", "notification_action_no_longer_relevant", {"analytics_event": False}, DB_PATH, SHEETS_WEBHOOK_URL)
            await bot_record_action_event(u, "skill_skipped", metadata={"source": "notification_no_longer_relevant"})
            await answer_with_keyboard(m, u, "Ок. Не возвращаю старый эксперимент. Что сейчас сделать?", kb_day_menu, "day_menu")
            return
        await restore_from_notification_context(m, u, source="notification_active_action")
        return

    if u.get("stage") == "notification_completed_action":
        if text == "Ещё один шаг":
            await log_event(uid, "notification", "notification_next_step_clicked", {"analytics_event": False}, DB_PATH, SHEETS_WEBHOOK_URL)
            await handle_action_request(uid, m, u, repeat=True)
            return
        if text == "Продолжить самостоятельно":
            await log_event(uid, "notification", "notification_continue_self_clicked", {"analytics_event": False}, DB_PATH, SHEETS_WEBHOOK_URL)
            await answer_with_keyboard(m, u, "Ок. Продолжай самостоятельно. Я рядом, если понадобится следующий шаг.", kb_day_menu, "day_menu")
            return
        if text == "Закрыть день":
            await start_day_review(m, u, "notification_completed_action")
            return
        await restore_from_notification_context(m, u, source="notification_completed_action")
        return

    if text in NOTIFICATION_BUTTONS and await restore_from_notification_context(m, u, source="notification_button"):
        return
    ...
    if text == "✏️ Уточнить ситуацию":
        set_legacy_stage(u, "analysis_action_extra_clarification")
        await save_user(u, DB_PATH)
    ...
    if u.get("stage") == "analysis_action_extra_clarification":
        ...
        set_legacy_stage(u, "analysis_action_transition")
        await save_user(u, DB_PATH)
        await handle_action_request(u["user_id"], m, u)
        return
    if text == "Не сейчас":
        set_legacy_stage(u, "analysis_action_postponed")
        await save_user(u, DB_PATH)
    ...
    if u.get("stage") == "extra_microstep":
        if text in {"✅ Сделал", "✅ Сделал(а)"} or ("сделал" in low and "не сделал" not in low):
            set_legacy_stage(u, "success_limit")
            set_current_state(u, STATE_PAUSED, close_action=True)
            await save_user(u, DB_PATH)
            await answer_with_keyboard(m, u, trainer_wrap(u, trainer_repeat_limit_text(u), "continue"), kb_success_limit, "success_limit")
            return
        if text in {"🟡 Не вышло", "🟡 Не получилось", "🟡 Попробовал, но не вышло"}:
            set_legacy_stage(u, "success_menu")
            set_current_state(u, STATE_PAUSED, close_action=True)
            await save_user(u, DB_PATH)
            await answer_with_keyboard(m, u, "Ок. Не дожимаем. Первый минимум уже засчитан — можно закрыть день или сменить навык.", kb_success_no_extra, "success_menu")
            return
    ...
    if (text in {"✅ Сделал", "✅ Сделал(а)"} or ("сделал" in low and "не сделал" not in low)) and (
        str(u.get("stage") or "") in {"downscale_action", "unclear_skill_simplified", "microstep", "downscale", "training"}
        or str(u.get("daily_skill_status") or "") in {"in_progress", "stuck"}
    ) and u.get("current_action_context") == "experiment":
        ...
        }, source="simplified_skill_done_recovery")
        await record_working_map_skill_result(u["user_id"], "completed_skills_effect_unknown", sid)
        await ask_skill_result_feedback(m, u, source="downscale_done")
        return
    ...
    if u.get("current_state") == STATE_AWAITING_RESULT and u.get("stage") not in {"skill_change_reason", "skill_change_free_text", "skill_change_meaning", "stuck_validation_choice", "stuck_micro_action", "stuck_effect_await", "stuck_aftercare", "consolidation", "consolidation_running"} and is_known_reply_button(text) and text not in ACTION_OUTCOME_BUTTONS and current_kind not in {"map", "crisis"}:
    ...
    if text == "▶️ Начать 3 минуты" and u.get("stage") == "consolidation":
        set_legacy_stage(u, "consolidation_running")
        await save_user(u, DB_PATH)
        await answer_with_keyboard(
            m,
            u,
    ...
    if u.get("stage") in {"experiment_completed_menu", "post_action_reflection"}:
        if text == "Продолжить задачу самому":
            await log_event(u["user_id"], "training", "experiment_continue_self", {"analytics_event": False}, DB_PATH, SHEETS_WEBHOOK_URL)
            await answer_with_keyboard(m, u, "Ок. Эксперимент закрыт. Продолжай задачу сам — без нового цикла отчёта.", kb_day_menu, "day_menu")
            return
        if text == "Ещё один маленький шаг":
            await log_event(u["user_id"], "training", "experiment_next_small_step", {"previous_attempt_id": active_attempt(u).get("attempt_id")}, DB_PATH, SHEETS_WEBHOOK_URL)
            await open_next_logical_step(m, u, source="experiment_completed_next_small_step")
            return
        if text == "Закрыть день":
            await start_day_review(m, u, "experiment_completed_menu")
            return
    ...
    if text in {"➕ Ещё 2 минуты", "💪 Закрепить ещё 2 минуты"}:
        ...
        set_legacy_stage(u, "extra_microstep")
        u["current_action_context"] = "daily_skill"
        await save_user(u, DB_PATH)
        await answer_with_keyboard(m, u, prompt, kb_extra_microstep_done, "extra_microstep")
        return

    if text in {"🌙 Закрыть день", "🌙 Закрыть подход", "🌙 На сегодня достаточно"}:
        set_legacy_stage(u, "waiting_next_day")
        set_current_state(u, STATE_PAUSED, close_action=True)
        await save_user(u, DB_PATH)
    ...
    if text in {"🧭 Следующий шаг", "💪 Другое действие", "💪 Продолжить тренировку"}:
    ...
    if text == "🗣️ Что помогло?":
        set_legacy_stage(u, "success_help_note")
        await save_user(u, DB_PATH)
    ...
    if u.get("stage") == "day_pause_confirm":
        if text == "✅ Закрыть день" or text == "🌙 Закрыть день":
            if day_closed_today(u):
                ...
                await answer_with_keyboard(m, u, DAY_ALREADY_CLOSED_TEXT, kb_day_core_stop, "day_core_stop")
                return
            await start_day_review(m, u, "day_pause_confirm_closed")
            return
    ...
    if u.get("stage") == "skill_change_reason":
        ...
        if text == "🤷 Не понимаю, зачем это делать":
            set_legacy_stage(u, "skill_change_meaning")
            await save_user(u, DB_PATH)
            await answer_with_keyboard(
                m,
                u,
        ...
        if text == "🎙️ Опишу ситуацию сам(а)":
            set_legacy_stage(u, "skill_change_free_text")
            await save_user(u, DB_PATH)
    ...
    # ---- Evening check-in: the same behavioural diary card as manual close ----
    if u.get("stage") == "evening_checkin":
        legacy_evening_answers = {
            "✅ сделал", "😐 частично", "❌ не сделал", "↩️ срывался, но возвращался",
            "🚪 Я всё-таки начал", "🟡 Пробовал, но застрял",
            "📱 Почти весь день уносило", "🫠 Не было сил",
        ...
        if text in DAY_REVIEW_FUNCTION_BY_BUTTON:
            ...
            u["last_active"] = time.time()
            u["pending_feedback_json"] = {
                "kind": "day_review",
                "source": "evening_checkin",
                "function": DAY_REVIEW_FUNCTION_BY_BUTTON[text],
            }
            set_legacy_stage(u, "day_review_barrier")
            await save_user(u, DB_PATH)
            await log_event(uid, "training", "evening_review_started", {"function": DAY_REVIEW_FUNCTION_BY_BUTTON[text]}, DB_PATH, SHEETS_WEBHOOK_URL)
            await answer_with_keyboard(m, u, "Что сильнее всего мешало именно в этот момент?", kb_day_review_barrier, "day_review_barrier")
            return
        if text in legacy_evening_answers:
            # Old Telegram keyboards can survive a deploy.  Do not guess the
            # function from their broad labels; move safely into the new card.
            await start_day_review(m, u, "legacy_evening_checkin")
            return
        await answer_with_keyboard(m, u, evening_checkin_text(), kb_evening_checkin, "evening_checkin")
        return
    ...
    if u.get("stage") == "await_training_target":
        ...
        if target_text.lower() != "пропустить":
            ...
            if task_needs_physical_step(target_text) and not u.get("current_next_physical_step"):
                set_legacy_stage(u, "await_task_physical_step")
                await save_user(u, DB_PATH)
                await m.answer(
    ...
    if user_is_in_action_loop(u):
        ...
        if text == "❌ Не сделал" or "не сделал" in low:
            ...
            set_current_state(u, STATE_AWAITING_STUCK_REASON)
            await save_user(u, DB_PATH)
            await bot_record_action_event(u, "skill_result_reported", skill_id=current_skill_id(u), metadata={"result_status": "not_completed_pending_reason", "button": text, "completed": False, "context_reason_required": True})
            await answer_with_keyboard(m, u, "Что помешало больше всего?", kb_skill_obstacle, "skill_obstacle")
            return
        ...
        if u.get("stage") == "failed_options":
            if text in {"😬 Страшно ошибиться / стыдно", "📱 Ушёл в телефон / YouTube", "🧱 Слишком много задач", "😰 Тревога или внутреннее напряжение", "🗯 Слишком сильно ругаю себя", "🫥 Не понимаю, зачем вообще делать", "😬 Страшно, стыдно, боюсь ошибиться", "🧠 Слишком много всего", "🔋 Нет сил", "🫨 Тревога и перегруз", "🧨 Самокритика после срыва", "🤷 Не понимаю, зачем это делать"}:
                ...
                return
            if text in {"✍️ Опишу сам(а)", "🎙️ Опишу голосом или текстом"}:
                set_legacy_stage(u, "stuck_reason_text")
                set_current_state(u, STATE_AWAITING_STUCK_REASON)
                await save_user(u, DB_PATH)
                await m.answer("Опиши как есть — одним сообщением или голосом.\nНе нужно формулировать красиво.\nЯ сначала попробую понять, что с тобой происходит, а не сразу дать совет.")
            ...
            if is_misunderstood(text):
                set_legacy_stage(u, "action_clarification")
                await save_user(u, DB_PATH)
                await answer_with_keyboard(
                    m,
                    u,
        ...
        if u.get("stage") == "stuck_micro_action":
            if text in {"✅ Сделал", "✅ Написал"} or ("сделал" in low and "не сделал" not in low) or "написал" in low:
                profile = await get_user_profile(u["user_id"], DB_PATH)
                pending_stuck = pending_stuck_micro_data(u)
                skill_id = pending_stuck.get("skill_id") or u.get("pending_stuck_skill_id") or current_skill_id(u) or DOWNSCALE_PRIMARY_SKILL
                previous_done = int(u.get("done_count") or 0)
                u["done_count"] = previous_done + 1
                mark_day_core_round_done(u)
                mark_current_skill_status(u, "completed")
                set_legacy_stage(u, "stuck_effect_await")
                await save_user(u, DB_PATH)
                await record_return_after_slip_action_event_if_needed(u, "stuck_micro_done")
                await record_return_after_disruption_if_needed(u, profile, "stuck_micro_done")
                await bot_record_action_event(u, "attempt_completed_self_reported", skill_id=skill_id, metadata={"source": "stuck_micro_action", "reason": pending_stuck.get("reason") or u.get("pending_stuck_reason") or ""})
                await record_profile_signal(u["user_id"], "training", {
                    "last_completed_skill": skill_id,
                    "last_skill_effect": "unknown",
                    "preferred_activation": "stuck_micro_step",
                ...
                }, source="stuck_micro_done")
                await record_development_avatar_event(u["user_id"], "skill_done", DB_PATH, {"skill_id": skill_id, "after_stuck": True, "reason": pending_stuck.get("reason") or u.get("pending_stuck_reason") or ""})
                await record_working_map_skill_result(u["user_id"], "completed_skills_effect_unknown", skill_id)
                await answer_with_keyboard(m, u, crisis_effect_prompt_text(), kb_crisis_effect, "stuck_effect_await")
                return
            if text == "😣 Не могу" or "не могу" in low:
                await log_event(u["user_id"], "training", "stuck_micro_blocked", {"reason": u.get("pending_stuck_reason") or ""}, DB_PATH, SHEETS_WEBHOOK_URL)
                await answer_with_keyboard(
                    m,
                    u,
                    "Ок. Ещё меньше: сделай только первую физическую часть — один выдох, один клик, убрать телефон из руки или открыть место задачи на 5 секунд.",
                    kb_crisis_action,
                    "stuck_micro_action",
                )
                return
            if text == "🧩 Ещё меньше" or "ещё меньше" in low or "еще меньше" in low:
                await log_event(u["user_id"], "training", "stuck_micro_downscale_requested", {"reason": u.get("pending_stuck_reason") or ""}, DB_PATH, SHEETS_WEBHOOK_URL)
                await answer_with_keyboard(
                    m,
                    u,
                    "Ещё меньше: не решай задачу. Только один микрошаг: выдох, глоток воды, свернуть экран, открыть файл или написать одну сырую строку.",
                    kb_crisis_action,
                    "stuck_micro_action",
                )
                return
            if text == "🆘 Мне всё ещё плохо" or "всё ещё плохо" in low or "все еще плохо" in low:
                ...
                return
            await answer_with_keyboard(m, u, "Сделай один микро-шаг и отметь: ✅ Сделал / 😣 Не могу / 🧩 Ещё меньше.", kb_crisis_action, "stuck_micro_action")
            return

        if u.get("stage") == "stuck_effect_await":
            effect = crisis_effect_code(text)
            profile = await get_user_profile(u["user_id"], DB_PATH)
            pending_stuck = pending_stuck_micro_data(u)
            skill_id = pending_stuck.get("skill_id") or u.get("pending_stuck_skill_id") or current_skill_id(u) or DOWNSCALE_PRIMARY_SKILL
            action_name = pending_stuck.get("action_name") or u.get("pending_stuck_action_name") or _skill_label(skill_id)
            total = int(profile.get("stuck_micro_effect_count") or 0) + 1
            success = int(profile.get("stuck_micro_effect_success_count") or 0) + (1 if effect == "better" else 0)
            await record_profile_signal(u["user_id"], "training", {
                "last_completed_skill": skill_id,
                "last_skill_effect": effect,
                "last_stuck_micro_action": action_name,
                "stuck_micro_effect_count": total,
                "stuck_micro_effect_success_count": success,
                "stuck_micro_success_rate": round(success / total, 2) if total else 0,
            }, source="stuck_micro_effect")
            await log_event(u["user_id"], "training", "stuck_micro_effect_recorded", {"effect": effect, "skill_id": skill_id, "action_name": action_name}, DB_PATH, SHEETS_WEBHOOK_URL)
            if effect == "worse":
                set_legacy_stage(u, "crisis_stabilize")
                await save_user(u, DB_PATH)
                await answer_with_keyboard(m, u, crisis_still_bad_text(), kb_crisis_stabilize, "crisis_stabilize")
                return
            set_legacy_stage(u, "stuck_aftercare")
            set_current_state(u, STATE_PAUSED, close_action=True)
            await save_user(u, DB_PATH)
            await answer_with_keyboard(
                m,
                u,
                f"Этот стопор сейчас удалось обойти через {action_name}.\n\nЧто тебе нужнее дальше?",
                kb_stuck_aftercare,
                "stuck_aftercare",
            )
            return

        if u.get("stage") == "stuck_aftercare":
            if text in {"➕ Ещё 2 минуты", "💪 Закрепить ещё 2 минуты"} or "закреп" in low:
                set_legacy_stage(u, "stuck_micro_action")
                pending_stuck = pending_stuck_micro_data(u)
                step = pending_stuck.get("micro_step") or u.get("pending_stuck_micro_step") or "повтори тот же микро-шаг ещё 2 минуты"
                await save_user(u, DB_PATH)
                await answer_with_keyboard(m, u, f"Закрепляем ещё 2 минуты.\n\n1. {step}", kb_crisis_action, "stuck_micro_action")
                return
            if text in {"🧭 Следующий шаг", "🧭 Вернуться к маршруту"} or "вернуться к маршруту" in low:
                set_legacy_stage(u, "training_main")
                u["pending_stuck_reason"] = None
                u["pending_stuck_action_name"] = None
                u["pending_stuck_skill_id"] = None
                u["pending_stuck_micro_step"] = None
                u["pending_plan_change"] = None
                await save_user(u, DB_PATH)
                await answer_with_keyboard(m, u, "Ок. Возвращаемся к маршруту — без автоматического старого навыка. Выбери следующий шаг.", kb_training_main, "training_main")
                return
            if text in {"🌙 Закрыть день", "🌙 На сегодня достаточно"} or "на сегодня достаточно" in low:
                u["pending_stuck_reason"] = None
                u["pending_stuck_action_name"] = None
                u["pending_stuck_skill_id"] = None
                u["pending_stuck_micro_step"] = None
                u["pending_plan_change"] = None
                await save_user(u, DB_PATH)
                await answer_with_keyboard(m, u, "Ок. На сегодня достаточно. Стопор записан как данные, не как провал.", kb_day_core_stop, "waiting_next_day")
                return
            await answer_with_keyboard(m, u, "Выбери, что тебе нужнее дальше:", kb_stuck_aftercare, "stuck_aftercare")
            return
        ...
        if u.get("stage") == "downscale_action":
            if text == "😣 Даже это сложно" or ("даже" in low and "сложно" in low):
                ...
                set_legacy_stage(u, "downscale_name_task")
                await save_user(u, DB_PATH)
    ...
    if u.get("stage") in {"waiting_next_day", "done", "day_core_stop"}:
        trainer_key = u.get("trainer_key") or "marsha"
        if text in {"📖 Полная карта", "🧭 Моя карта", "🧭 Посмотреть карту"} or "моя карта" in low or "посмотреть карту" in low or "полная карта" in low:
        ...
        if text == "📚 Почему это работает" or "почему это работает" in low:
            ...
            return
        if text in {"✅ Завершить", "🌙 До завтра"} or "до завтра" in low:
        ...
        if (text == "📌 Что изменилось?" or "что изменилось" in low) and u.get("stage") != "day_core_stop":
            set_legacy_stage(u, "after_action_note")
            await save_user(u, DB_PATH)
            await log_event(u["user_id"], "training", "after_action_note_requested", {}, DB_PATH, SHEETS_WEBHOOK_URL)
            await m.answer("Напиши 1–5 слов или пришли голосовое: что изменилось после шага?")
            return
        if text == "➕ Ещё один короткий шаг" or ((text == "🔁 Ещё круг" or "еще круг" in low or "ещё круг" in low) and u.get("stage") != "day_core_stop"):
            await handle_action_request(u["user_id"], m, u, repeat=True)
            return
        if text in {"🌙 На сегодня хватит", "🌙 Хватит на сегодня"} or "хватит" in low:
            set_legacy_stage(u, "day_pause_confirm")
    ...
    if u["stage"] == "await_input_mode":
        ...
        if text == "🎙 Диагностика голосом" or "голос" in low:
            u["input_mode"] = "voice"
            set_legacy_stage(u, "await_problem_voice")
            await save_user(u, DB_PATH)
        ...
        if text == "❓ Быстрый тест (5 вопросов)" or "тест" in low:
            u["input_mode"] = "test"
            set_legacy_stage(u, "taking_test")
            u["test_answers"] = []
            await save_user(u, DB_PATH)
    ...
    if u["stage"] == "await_problem_voice":
        if text and text.lower() == "назад":
            set_legacy_stage(u, "choose_input_mode")
            await save_user(u, DB_PATH)
    ...
    if u.get("stage") == "misunderstood_reason":
        ...
        if text.startswith("5") or "объяснить иначе" in low or "по-другому" in low or "по другому" in low:
            ...
            set_legacy_stage(u, "misunderstood_explain_await")
            await save_user(u, DB_PATH)
            await log_event(u["user_id"], "analysis", "misunderstood_reason_selected", {"reason": reason}, DB_PATH, SHEETS_WEBHOOK_URL)
            await m.answer(misunderstood_single_question(reason))
            return
    ...
    # Legacy reply-keyboard fallback for barrier screens sent before inline callbacks.
    if u.get("stage") in {"analysis_need_more", "awaiting_barrier_choice"}:
        if not text:
        ...
        if text in BARRIER_BUTTONS:
            await continue_analysis_from_barrier(m, u, text)
            return
        await answer_with_keyboard(m, u, "Выбери вариант кнопкой — так я не запущу анализ повторно.", kb_analysis_need_more, "analysis_need_more")
        return
    ...
    if u.get("stage") == "training":
        ...
        if text == "✅ Сделал(а)" or ("сделал" in low and "не сделал" not in low):
            if u.get("current_action_context") != "experiment":
                await show_action_changed_fallback(m, u, "done_without_active_experiment")
                return
    ...
    if u.get("stage") == "crisis_stabilize":
        ...
        if text == "✅ Сделал" or low == "сделал":
            if u.get("current_action_context") != "crisis_stabilization":
                await answer_with_keyboard(m, u, "Этот экран уже не актуален.", kb_crisis_stabilize, "crisis_stabilize")
                return
        ...
        if text == "✍️ Написать, что происходит" or "написать" in low:
            set_legacy_stage(u, "crisis_text")
            await save_user(u, DB_PATH)
    ...
    if u.get("stage") == "crisis_action_await":
        ...
        if text in {"✅ Сделал", "✅ Написал"} or "сделал" in low or "написал" in low or "вернулся" in low:
            if u.get("current_action_context") != "crisis_stabilization":
                await answer_with_keyboard(m, u, "Этот экран уже не актуален.", kb_crisis_action, "crisis_action")
                return
            profile = await get_user_profile(u["user_id"], DB_PATH)
    ...
    if u.get("stage") == "offer":
        ...
        if text in {"🤔 Остаться в коротком режиме", "🤔 Подумаю"} or "коротком режиме" in low or "подумаю" in low:
            ...
            u["free_mode"] = 1
            u["payment_status"] = "free_mode"
            set_legacy_stage(u, "feedback_offer")
            await save_user(u, DB_PATH)
            await answer_with_keyboard(
                m,
                u,
    ...
    # техническое восстановление маршрута: засчитываем микрошаг и сразу спрашиваем эффект.
    raw_stage = str(u.get("stage") or "")
    if (text in {"✅ Сделал", "✅ Сделал(а)"} or ("сделал" in low and "не сделал" not in low)) and (
        raw_stage in {"downscale_action", "unclear_skill_simplified", "microstep", "downscale", "training"}
        or str(u.get("daily_skill_status") or "") in {"in_progress", "stuck"}
    ) and u.get("current_action_context") == "experiment":
        previous_done = int(u.get("done_count") or 0)
        u["done_count"] = previous_done + 1
    ...
    # возвращаем к безопасному меню текущего дня без технических формулировок.
...
ACTIVE_SKILL_STAGES = {
    ...
    "downscale", "downscale_action", "stuck_micro_action", "stuck_effect_await", "stuck_aftercare", "skill_not_tried", "skill_change_reason",
...
kb_notification_active_action = ReplyKeyboardMarkup(
    keyboard=[
        [KeyboardButton(text="Продолжить")],
        [KeyboardButton(text="Сделать проще")],
        [KeyboardButton(text="Другой способ")],
        [KeyboardButton(text="Больше не актуально")],
    ],
    resize_keyboard=True,
)

kb_notification_completed_action = ReplyKeyboardMarkup(
    keyboard=[
        [KeyboardButton(text="Ещё один шаг")],
        [KeyboardButton(text="Продолжить самостоятельно")],
        [KeyboardButton(text="Закрыть день")],
    ],
    resize_keyboard=True,
)
...
NOTIFICATION_BUTTONS = {
    "Продолжить", "Сделать проще", "Другой способ", "Больше не актуально",
    "Ещё один шаг", "Продолжить самостоятельно", "Закрыть день",
}

def notification_type_from_keyboard(keyboard_name: str) -> str:
    if "reactivation" in str(keyboard_name or ""):
        return "reactivation"
    if "evening" in str(keyboard_name or ""):
        return "evening"
    return "morning"

def active_notification_context(u: Dict[str, Any]) -> Dict[str, Any]:
    raw = u.get("last_notification_context")
    if isinstance(raw, str):
        try:
            raw = json.loads(raw)
        except Exception:
    ...
    if expires and expires < dt.datetime.now(dt.timezone.utc):
        return {"expired": True, **raw}
    return raw

def remember_notification_context(u: Dict[str, Any], keyboard_name: str) -> Dict[str, Any]:
    now = dt.datetime.now(dt.timezone.utc)
    attempt = active_attempt(u)
    ctx = {
        "notification_id": f"notif_{uuid.uuid4().hex}",
        "user_id": str(u.get("user_id") or ""),
        "notification_type": notification_type_from_keyboard(keyboard_name),
        "target_stage": str(u.get("stage") or ""),
        "day_id": str(u.get("current_day_id") or ""),
        "attempt_id": str(attempt.get("attempt_id") or ""),
        "skill_id": str(current_skill_for_action(u) or u.get("daily_skill_id") or ""),
        "active_action": bool(current_skill_for_action(u) and not attempt.get("is_closed") and not attempt.get("day_closed")),
        "sent_at": now.isoformat(),
        "expires_at": (now + dt.timedelta(hours=36)).isoformat(),
    }
    u["last_notification_context"] = ctx
    return ctx

async def open_safe_day_menu_from_notification(m: Message, u: Dict[str, Any], reason: str) -> None:
    await log_event(u["user_id"], u.get("stage", ""), "notification_safe_menu_opened", {"reason": reason, "analytics_event": False}, DB_PATH, SHEETS_WEBHOOK_URL)
    if day_closed_today(u):
        await answer_with_keyboard(m, u, DAY_ALREADY_CLOSED_TEXT, kb_day_core_stop, "day_core_stop")
        return
    await answer_with_keyboard(m, u, "Что сейчас сделать?", kb_day_menu, "day_menu")

async def restore_from_notification_context(m: Message, u: Dict[str, Any], *, source: str) -> bool:
    ctx = active_notification_context(u)
    if not ctx:
        return False
    if ctx.get("expired"):
        await open_safe_day_menu_from_notification(m, u, "expired")
        return True
    if day_closed_today(u):
        await open_safe_day_menu_from_notification(m, u, "day_closed")
        return True
    attempt = active_attempt(u)
    skill_id = str(ctx.get("skill_id") or current_skill_for_action(u) or "")
    if ctx.get("day_id") and not u.get("current_day_id"):
        u["current_day_id"] = ctx.get("day_id")
    if skill_id and skill_id in SKILLS_DB:
        u["current_skill"] = skill_id
        u["daily_skill_id"] = u.get("daily_skill_id") or skill_id
    if ctx.get("active_action") and not attempt.get("is_closed"):
        set_legacy_stage(u, "notification_active_action")
        await save_user(u, DB_PATH)
        await log_event(u["user_id"], "notification", "notification_context_restored", {**ctx, "source": source, "analytics_event": False}, DB_PATH, SHEETS_WEBHOOK_URL)
        await m.answer("Вы остановились на этом эксперименте.", reply_markup=kb_notification_active_action)
        return True
    set_legacy_stage(u, "notification_completed_action")
    await save_user(u, DB_PATH)
    await log_event(u["user_id"], "notification", "notification_context_restored", {**ctx, "source": source, "analytics_event": False}, DB_PATH, SHEETS_WEBHOOK_URL)
    await m.answer("Эксперимент уже выполнен. Что дальше?", reply_markup=kb_notification_completed_action)
    return True
...
def sync_active_attempt(
    ...
    *,
    ...
    if attempt_status in ATTEMPT_STATUSES:
        ...
        if attempt_status == "not_tried" and is_closed is False and u.get("current_action_id"):
            attempt["attempt_id"] = str(u.get("current_action_id"))
...
async def claim_callback_once(c: CallbackQuery, u: Dict[str, Any], source: str) -> bool:
    """Return True only for the first processing attempt of a Telegram callback."""
    callback_id = str(getattr(c, "id", "") or "")
    message_id = str(getattr(getattr(c, "message", None), "message_id", "") or "")
    if not callback_id:
        return True
    day_id = str(u.get("current_day_id") or "")
    attempt_id = str(active_attempt(u).get("attempt_id") or "")
    state_version = int(u.get("state_version") or 0)
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "CREATE TABLE IF NOT EXISTS processed_callbacks (callback_query_id TEXT PRIMARY KEY, user_id INTEGER, message_id TEXT, day_id TEXT, attempt_id TEXT, state_version INTEGER, source TEXT, created_at TEXT)"
        )
        try:
            await db.execute(
                "INSERT INTO processed_callbacks(callback_query_id, user_id, message_id, day_id, attempt_id, state_version, source, created_at) VALUES(?,?,?,?,?,?,?,?)",
                (callback_id, u.get("user_id"), message_id, day_id, attempt_id, state_version, source, utc_iso()),
            )
            await db.commit()
            return True
        except aiosqlite.IntegrityError:
            await db.rollback()
    await log_event(u.get("user_id"), u.get("stage", ""), "duplicate_callback_ignored", {
...
async def validate_callback_screen(c: CallbackQuery, u: Dict[str, Any], source: str) -> tuple[bool, str]:
    ...
    if not await claim_callback_once(c, u, source):
        return False, action
...
async def on_prediction_callback(c: CallbackQuery):
    ...
    prepare_calendar_routing(u)
    try:
        _, prediction_id, result = (c.data or "").split(":", 2)
    except ValueError:
        await reject_lost_callback(c, u, "prediction_malformed")
        return
    profile = await get_user_profile(u["user_id"], DB_PATH)
    if prediction_id != str(profile.get("day1_prediction_id") or "") or profile.get("day1_prediction_status") != "UNTESTED":
        await reject_stale_callback(c, u, "prediction_stale")
        return
    status = {"yes": "SUPPORTED", "partial": "PARTIAL", "no": "NOT_SUPPORTED", "unknown": "INCONCLUSIVE"}.get(result)
    if not status:
        await reject_lost_callback(c, u, "prediction_result_invalid")
        return
    profile_patch = {
        "day1_prediction_status": status,
        "day1_prediction_checked_at": dt.datetime.now(dt.timezone.utc).isoformat(),
    }
    try:
        conclusion = model_from_dict(profile.get("conclusion_model") or {})
        conclusion = update_next_untested_prediction(
            conclusion,
            {"yes": "STRONG_SUCCESS", "partial": "WEAK_SUCCESS", "no": "EXECUTED_ONLY", "unknown": "UNKNOWN"}[result],
            experiment_name="проверка прогноза следующего дня",
            result_detail=f"Пользователь отметил прогноз: {result}.",
        )
        profile_patch["conclusion_model"] = conclusion.as_dict()
    except (KeyError, TypeError, ValueError):
        pass
    await update_user_profile(u["user_id"], profile_patch, DB_PATH, source="prediction_check")
    await log_event(u["user_id"], "prediction", "prediction_tested", {"prediction_id": prediction_id, "result": result, "status": status}, DB_PATH, SHEETS_WEBHOOK_URL)
    await c.message.answer("Записал. Это обновит рабочую модель; новый анализ для ответа не нужен.", reply_markup=action_keyboard())
    await c.answer()

@router.callback_query(lambda c: (c.data or "").startswith("barrier:"))
async def on_barrier_callback(c: CallbackQuery):
    u = await get_user(c.from_user.id, DB_PATH)
    prepare_calendar_routing(u)
    await save_user(u, DB_PATH)
    if not await claim_callback_once(c, u, "barrier_choice"):
        return
    try:
        _, analysis_id, barrier = (c.data or "").split(":", 2)
    except ValueError:
        await reject_lost_callback(c, u, "barrier_malformed")
        return
    current = str(u.get("current_screen_id") or "")
    if u.get("stage") != "awaiting_barrier_choice" or not current or analysis_id != current or barrier not in {v[0] for v in BARRIER_BUTTONS.values()}:
        await reject_stale_callback(c, u, "barrier_screen_mismatch")
        return
    await continue_analysis_from_barrier(c.message, u, barrier)
    await c.answer()
...
async def on_offer_callbacks(c: CallbackQuery):
    ...
    click_event = {
        ...
        OFFER_CALLBACKS["group"]: "offer_group_clicked",
        OFFER_CALLBACKS["live"]: "offer_human_clicked",
    }.get(data)
    if click_event:
    ...
    if data in {OFFER_CALLBACKS["request_live"], OFFER_CALLBACKS["request_guided"], OFFER_CALLBACKS["request_group"]}:
        ...
        await c.message.answer(
            offer_request_form_text(format_label, has_telegram_contact=bool(getattr(c.from_user, "username", None))),
    ...
    if data == OFFER_CALLBACKS["payment_claim"]:
        u["payment_status"] = "pending_manual_review"
        u["last_payment_click"] = data
        await save_user(u, DB_PATH)
        await log_event(
            uid, "offer", "payment_review_requested",
            {"source": "subscription_offer", "amount": float(BASE_OFFER_EUR)},
            DB_PATH, SHEETS_WEBHOOK_URL,
        )
        sent = await notify_offer_request(
            c, u, "payment_review",
            f"Пользователь сообщает об оплате подписки €{BASE_OFFER_EUR_LABEL}. Проверь платёж и выполни /mark_paid {uid}.",
        )
        if sent:
            await c.message.answer(
                "Заявка на проверку отправлена Ивану. После сверки платежа он включит полный режим вручную."
            )
        else:
            contact = curator_contact_url() or "@Ivan_Vasiliuk"
            await c.message.answer(
    ...
    if data in {"pay:bot_999", "pay:live_59", "pay:guided_149"}:
        ...
        if pay_url and ENABLE_PAYMENTS:
            ...
            u["last_payment_click"] = data
            await save_user(u, DB_PATH)
            await log_event(uid, "offer", "payment_link_opened", {"source": data, "amount": amount_label}, DB_PATH, SHEETS_WEBHOOK_URL)
            await c.message.answer(
                "Оплата доступна по кнопке ниже. После оплаты вернись в карточку тарифа и нажми «Уже оплатил(а) — проверить».",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"💳 Оплатить {amount_label}", url=pay_url)]]),
            )
        else:
            await c.message.answer(payment_not_ready_text())
        await c.answer()
    ...
    if data in {"curator_path", "specialist_path"}:
        profile = await get_user_profile(uid, DB_PATH)
        profile["_skill_map"] = await build_skill_map_data(u, profile)
        set_legacy_stage(u, "curator_path")
...
async def on_test_answer(c: CallbackQuery):
    ...
    try:
        ...
        if len(test_answers) < len(TEST_QUESTIONS):
            ...
            u["test_answers"] = []
            set_legacy_stage(u, "test_complete_show_analysis")
            await save_user(u, DB_PATH)
...
kb_reminder_overload = ReplyKeyboardMarkup(
    keyboard=[
        [KeyboardButton(text="Оставить одно в день")],
        [KeyboardButton(text="Только утром")],
...
def reminder_mode(u: Dict[str, Any]) -> str:
    return str(u.get("reminder_mode") or "evening_only")
...
def reminder_mode_allows(u: Dict[str, Any], notification_type: str, today: str) -> bool:
    ...
    if mode == "evening_only" and notification_type != "evening":
        return False
    if mode == "one_per_day" and _proactive_count_today(u, today) >= 1:
        return False
    return True
...
async def send_background_keyboard(bot: Bot, u: Dict[str, Any], text: str, reply_markup, keyboard_name: str):
    ...
    await log_event(
        ...
        {"keyboard": keyboard_name, "button_count": button_count, "source": "background", "notification_id": notification_context.get("notification_id")},
        DB_PATH,
        SHEETS_WEBHOOK_URL,
    )
    try:
...
MAX_PROACTIVE_PER_DAY = 2  # hard product cap across every proactive message
            ...
            for row in rows:
                ...
                if in_time_window(now_local, *MORNING_REMINDER_WINDOW) and u.get("last_morning_checkin_date") != today:
                    if not reminder_mode_allows(u, "morning", today):
                        continue
                    if should_ask_reminder_overload(u):
                    ...
                    if closed_today:
                        ...
                        continue
                    u["last_morning_checkin_date"] = today
                    set_legacy_stage(u, "morning_new_day" if is_closed else "morning_checkin")
                    mark_bot_auto_message(u)
                ...
                if in_time_window(now_local, *EVENING_REMINDER_WINDOW) and u.get("last_evening_checkin_date") != today:
                    if not reminder_mode_allows(u, "evening", today):
                        continue
                    if closed_today:
                        # Closing the day is an explicit opt-out from every
                        # remaining proactive message on that local date.
                        u["last_evening_checkin_date"] = today
                        await save_user(u, DB_PATH)
                        await log_closed_day_auto_block(u, "evening_same_calendar_day", {"today": today})
                        continue
                    if should_ask_reminder_overload(u):
                        await ask_reminder_overload(bot, u)
                        continue
                    ...
                    set_legacy_stage(u, "evening_checkin" if day_active else "evening_not_started")
                    mark_bot_auto_message(u)
                    _increment_proactive_count(u, today)
                    await save_user(u, DB_PATH)
                ...
                if should_skip_reactivation_for_closed_day(u, today):
                    continue
                if is_closed:
                    continue
                if not reminder_mode_allows(u, "reactivation", today):
                    continue
                if should_ask_reminder_overload(u):
                    await ask_reminder_overload(bot, u)
                    continue
...
async def main() -> int:
    ...
    try:
        ...
        await startup_schema_check(DB_PATH)
        if SKILL_REGISTRY_ENABLED:
            ...
            production_count = ACTIVE_FILE_SKILL_REGISTRY.contour_counts()["production"]
            if SKILL_LIBRARY_FAIL_CLOSED and not 30 <= production_count <= 50:
                raise RuntimeError(
                    "Production skill contour must contain 30..50 reviewed cards; "
                    f"found {production_count}"
                )
