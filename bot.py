...
async def finalize_skill_result_feedback(m: Message, u: Dict[str, Any], text: str, result_status: str, effect: str, effect_rating: int, *, completed: bool, send_menu: bool = True) -> bool:
    ...
    if send_menu:
        ...
        if completed and not has_seen_mini_lesson_today(u):
            today = local_date_for_user(u)
            u["last_mini_lesson_date"] = today
            async with aiosqlite.connect(DB_PATH) as _db:
                await _db.execute(
                    "UPDATE users SET last_mini_lesson_date = ? WHERE user_id = ?",
                    (today, u["user_id"]),
                )
                await _db.commit()
            await m.answer(mini_lesson_text(sid or ""))
    return True
...
# ============================================================
# МИНИ-УРОКИ — показываются один раз в день после выполнения навыка
# ============================================================

MINI_LESSONS: Dict[str, str] = {
    "open_only": "📚 Мини-урок: Физический контакт с задачей (открыть файл, папку, страницу) без цели что-то сделать — это уже нейронный сигнал. Мозг начинает снижать избегание после повторных безопасных контактов с тем же местом.",
    "open_without_timer": "📚 Мини-урок: «Открыть без таймера» тренирует разрыв между «место задачи» и «обязательство работать». Это самый дешёвый вход: мозг соглашается, потому что не слышит обещания марафона.",
    "task_naming": "📚 Мини-урок: Называние переводит расплывчатое «надо что-то сделать с чем-то» в конкретный объект. Одно слово снижает когнитивный туман — и избегание вместе с ним.",
    "ninety_sec_start": "📚 Мини-урок: ADHD-мозгу сложно согласиться на весь путь, но легко — на 90 секунд. Короткий контейнер убирает угрозу масштаба и тренирует запуск как отдельный навык.",
    "two_min_start": "📚 Мини-урок: Две минуты действия запускают поведенческую инерцию. После старта продолжить примерно в 4 раза легче, чем начать заново с нуля.",
    "phone_far_3min": "📚 Мини-урок: Телефон рядом снижает когнитивный ресурс даже когда ты в него не смотришь. Расстояние — это не дисциплина, а изменение среды, которое работает автоматически.",
    "phone_away_3_min": "📚 Мини-урок: Телефон рядом снижает когнитивный ресурс даже когда ты в него не смотришь. Расстояние — это не дисциплина, а изменение среды, которое работает автоматически.",
    "bad_first_step": "📚 Мини-урок: «Плохой черновик» разрывает связь «начало = публичный результат». Снимая требование идеального старта, мозг перестаёт воспринимать вход как угрозу оценки.",
    "bad_draft": "📚 Мини-урок: «Плохой черновик» разрывает связь «начало = публичный результат». Снимая требование идеального старта, мозг перестаёт воспринимать вход как угрозу оценки.",
    "body_before_task": "📚 Мини-урок: Нервная система на высоком возбуждении хуже справляется с когнитивными задачами. Один телесный ритуал перед задачей — это не промедление, а подготовка контейнера для работы.",
    "body_first": "📚 Мини-урок: Нервная система на высоком возбуждении хуже справляется с когнитивными задачами. Один телесный ритуал перед задачей — это не промедление, а подготовка контейнера для работы.",
    "one_breath": "📚 Мини-урок: Один длинный выдох активирует парасимпатическую нервную систему быстрее, чем любая техника расслабления. Тело первым, потом задача.",
    "restart_after_slip": "📚 Мини-урок: Возврат без самонаказания — это сам по себе навык. Срыв + быстрый нейтральный возврат обучает мозг: срыв = информация, а не катастрофа.",
    "restart_after_break": "📚 Мини-урок: Возврат без самонаказания — это сам по себе навык. Срыв + быстрый нейтральный возврат обучает мозг: срыв = информация, а не катастрофа.",
    "check_the_facts_light": "📚 Мини-урок: Мысль «я не могу это сделать» — гипотеза, не факт. Проверяя факты вместо приговора, мы снижаем стоимость входа и возвращаем выбор.",
    "self_criticism_to_instruction": "📚 Мини-урок: Самокритика активирует центры угрозы и усиливает избегание. Переформулировка в инструкцию переключает мозг из режима самозащиты в режим действия.",
    "one_tab_focus": "📚 Мини-урок: Каждая открытая вкладка — источник прерывания. Мозг тратит ресурс на игнорирование стимулов, даже глядя в одно место. Одна вкладка — это не ограничение, а освобождение ресурса.",
    "urge_surf_60": "📚 Мини-урок: Импульс прокрастинации длится от 30 до 90 секунд на пике. Пауза-«сёрфинг» не подавляет импульс — она позволяет ему пройти, и тогда действие становится выбором.",
    "visible_next_step": "📚 Мини-урок: Внешняя подсказка заменяет удержание намерения в рабочей памяти. Для мозга с дефицитом рабочей памяти видимый шаг — это буквально внешний жёсткий диск.",
    "minimum_viable_day": "📚 Мини-урок: Минимум дня — не снижение требований. Это защита от «всё или ничего». Один шаг в плохой день держит нейронную цепочку действия активной.",
    "if_then_plan": "📚 Мини-урок: Правило «если X, то Y» переносит решение с момента X (когда ресурса меньше) на момент написания правила (когда ресурса больше). Это снижает трение входа.",
    "body_doubling_plan": "📚 Мини-урок: Молчаливое присутствие другого человека активирует социальные нейроны и повышает способность к запуску задачи. Это нейробиологический феномен, а не психологический трюк.",
}

_DEFAULT_MINI_LESSON = "📚 Мини-урок: Поведенческие навыки работают через повторение, а не через моментальное «понял». Каждый маленький вход — тренировка нейронного пути запуска, даже если результат кажется незначительным."


def mini_lesson_text(skill_id: str) -> str:
    """Return a brief educational mini-lesson for the given skill_id."""
    return MINI_LESSONS.get(str(skill_id or ""), _DEFAULT_MINI_LESSON)


def has_seen_mini_lesson_today(u: Dict[str, Any]) -> bool:
    """True if the user already received their daily mini-lesson today."""
    return str(u.get("last_mini_lesson_date") or "") == local_date_for_user(u)


# Day-specific engagement hooks for the first 3 days
EARLY_DAY_HOOKS: Dict[int, str] = {
    1: "💡 День 1: Мы не «начинаем новую жизнь». Мы просто пробуем один маленький вход — без требования продолжения.",
    2: "💡 День 2: Вчерашний вход дал первый сигнал мозгу, что начинать безопасно. Сегодня закрепляем паттерн.",
    3: "💡 День 3: Три входа подряд — первый нейробиологический след формирующейся привычки. После сегодня риск прервать привычку заметно снижается.",
}
