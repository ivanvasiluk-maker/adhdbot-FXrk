...
kb_crisis_tool_select = ReplyKeyboardMarkup(
    keyboard=[
        ...
        [KeyboardButton(text="7️⃣ Другое")],
    ],
    resize_keyboard=True,
)
...
def crisis_tool_limit_text() -> str:
    return (
        "Сегодня лимит кризисных подборов исчерпан.\n\n"
        "Используй основной навык дня."
    )


def crisis_tool_text(reason: str) -> str:
    tools = {
        "cannot_start": (
            "🧩 Аварийный запуск\n\n"
            "Не работай.\n\n"
        ...
        "stuck": (
            ...
            "Положи телефон дальше руки.\n\n"
            "Сделай один клик в сторону задачи.\n\n"
            "Только один."
        ),
        "fear_error": (
            "🧩 Плохой черновик\n\n"
            "Разрешено сделать плохо.\n\n"
            "Нельзя делать хорошо.\n\n"
            "Сейчас только черновик."
        ),
        "too_big": (
            "🧩 Резак задачи\n\n"
            "Не задача.\n\n"
    ...
    return tools.get(reason, tools["other"])
